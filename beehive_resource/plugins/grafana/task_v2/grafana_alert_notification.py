...
class GrafanaAlertNotificationTask(AbstractResourceTask):
    ...
    def grafana_alert_notification_create_physical_step(task, step_id, params, *args, **kvargs):
        ...
        try:
            ...
            task.progress(
                step_id,
                msg=" Grafana alert_notification %s already exist - inst_id: %s" % (name, inst_id),
            )
        except:
    ...
    def grafana_alert_notification_delete_physical_step(task, step_id, params, *args, **kvargs):
        ...
        if resource.is_ext_id_valid() is True:
            try:
                ...
                task.progress(
                    step_id,
                    msg=" Grafana alert_notification deleted %s" % resource.ext_id,
                )
            except:
                task.progress(
                    step_id,
                    msg=" Grafana alert_notification %s does not exist anymore" % resource.ext_id,
                )
