...
def router_create_entity(self, options):
    ...
    **Return:**
    ...
    # create openstack router
    inst = conn.network.router.create(name, project_extid, network_extid, 
                                      external_ips, routes)
    inst_id = inst['id']
    ...
    if ext_id is not None:
        ...
        for port in ports:
            ...
            if res is not None:
                ...
                self.update('PROGRESS', msg='Delete router port %s resource' % 
                            port['id'])
...
def router_port_create_entity(self, options):
    ...
    **Return:**
    ...
    p = container.add_resource(objid=objid, name=name, 
            resource_class=OpenstackPort, ext_id=ext_id, 
            active=True, desc=desc, attrib={}, parent=parent.oid, 
            tags=['openstack', 'port'])
    container.update_resource_state(p.id, ResourceState.ACTIVE)    
