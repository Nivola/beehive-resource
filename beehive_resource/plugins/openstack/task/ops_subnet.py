...
def subnet_create_dhcp_ports(self, options):
    ...
    **Return:**
    ...
    for port in ports:
        ...
        p = container.add_resource(objid=objid, name=name, 
                resource_class=OpenstackPort, ext_id=port['id'], 
                active=True, desc=desc, attrib={}, parent=network.oid, 
                tags=['openstack', 'port'])
...
def subnet_update_entity(self, options):
    ...
    **Return:**
    ...
    container = self.get_container(cid)
    conn = container.conn
    inst = conn.network.subnet.update(oid, name, None, None, 
                                      gateway_ip, None, allocation_pools, 
                                      enable_dhcp, host_routes, 
                                      dns_nameservers)
    inst_id = inst['id']
