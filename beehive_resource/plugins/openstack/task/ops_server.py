...
class ServerHelper(object):
    ...
    def create_volume(self, name, desc, parent_id, project_extid, availability_zone, config, server, boot=False):
        ...
        if source_type in ['image', 'snapshot', None]:
            ...
            volume_ext = self.conn.volume.create(size=int(volume_size), availability_zone=availability_zone,
...
def task_server_set_flavor(self, options):
    ...
    res = server_action(self, action, 'Set flavor to server', 'Error setting flavor to server',
                        final_status='ACTIVE')
    return res
