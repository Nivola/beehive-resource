...
class VolumeTask(AbstractResourceTask):
    ...
    def volume_create_physical_step(task, step_id, params, *args, **kvargs):
        ...
        if source_volid is not None:
            ...
            if source_volume.container_id != cid:
                ...
                task.progress(step_id, msg='clone openstack volume %s to %s' % (source_volume.ext_id, clone_volume_id))
                ...
                backend_host = backend['name']
                final_volume = container.conn.volume_v3.manage(migrated_clone_volume_id, name,
                                                               dest_intermediary_volume_type.ext_id,
                                                               bootable=source_volume.is_bootable(), desc=desc,
                ...
                task.progress(step_id, msg='attach openstack volume %s to volume %s' % (volume_id, oid))
                ...
                while cond is True:
                    ...
                    sleep(2)
                task.progress(step_id, msg='manage openstack volume %s on container %s' %
                                           (migrated_clone_volume_id, container.oid))
                ...
                task.progress(step_id, msg='unmanage openstack volume %s on container %s' %
        ...
        # create volume from image
            ...
            volume_type = task.get_resource(volume_type_id)
            volume_ext = conn.volume_v3.create(size=int(size), availability_zone=availability_zone,
                                               source_volid=source_volid, description=desc, multiattach=False,
                                               snapshot_id=snapshot_id, name=name, imageRef=image,
                                               volume_type=volume_type.ext_id, metadata=metadata, source_replica=None,
                                               consistencygroup_id=consistencygroup_id, scheduler_hints=None,
            ...
            for snapshot in snapshots:
                ...
                while True:
                    try:
                        ...
                        task.progress(step_id, msg='Volume %s snapshot %s deleted' % (ext_id, snapshot['id']))
                        break
    ...
    @staticmethod
    def volume_action(task, step_id, action, success, error, params, final_status='ACTIVE', projectid=None):
    ...
    def volume_set_flavor_step(task, step_id, params, *args, **kvargs):
        ...
        res = VolumeTask.volume_action(task, step_id, set_flavor_action, 'Set flavor to volume',
                                       'Error setting flavor to volume', params, final_status='ORIGINAL')
        return res, params
