...
class ServerHelper(object):
    ...
    def create_volume(self, name, desc, parent_id, project_extid, availability_zone, config, server, boot=False):
        ...
        if source_type in ['image', 'snapshot', None]:
            ...
            volume = self.container.add_resource(objid=objid, name=name, resource_class=OpenstackVolume, ext_id=None,
                                                 active=False, desc=desc, attrib={'size': volume_size},
                                                 parent=parent_id, tags=['openstack', 'volume'])
            self.task.progress(self.step_id, msg='create volume resource %s' % volume.id)

            # link volume id to server
            server.add_link('%s-%s-volume-link' % (server.oid, volume.id), 'volume', volume.id,
                            attributes={'boot': boot})
            self.task.progress(self.step_id, msg='setup volume link from %s to server %s' % (volume.id, server.oid))
            ...
            volume_ext = self.conn.volume_v3.create(size=int(volume_size), availability_zone=availability_zone,
                                                    source_volid=None, description=desc, multiattach=False,
                                                    snapshot_id=snapshot, name=name, imageRef=image,
                                                    volume_type=volume_type, metadata=None,
                                                    source_replica=None, consistencygroup_id=None, scheduler_hints=None,
            ...
            self.task.progress(self.step_id, msg='Attach openstack volume %s to volume %s' % (volume_id, volume.id))

            # loop until entity is not stopped or get error
            while True:
                inst = OpenstackVolume.get_remote_volume(self.container.controller, volume_id, self.container, 
                                                         volume_id)
                status = inst.get('status', 'error')
                if status == 'available':
                    break
                elif status == 'error':
                    self.task.progress(self.step_id, msg='create openstack volume %s - Error' % volume_id)
        ...
        # get existing volume
            ...
            if volume_uuid is not None:
                ...
                if volume_clone is True:
                    ...
                    final_volume_resource = self.container.add_resource(objid=objid, name=name,
                        resource_class=OpenstackVolume, ext_id=None, active=False, desc=desc,
                        attrib={'size': volume_size}, parent=parent_id, tags=['openstack', 'volume'])
                    self.task.progress(self.step_id, msg='create volume resource %s' % final_volume_resource.id)

                    # link volume id to server
                    server.add_link('%s-%s-volume-link' % (server.oid, final_volume_resource.id), 'volume',
                                    final_volume_resource.id, attributes={'boot': boot})
                    self.task.progress(self.step_id, msg='Setup volume link from %s to server %s' %
                                                         (final_volume_resource.id, server.oid))
                    ...
                    if volume.container_id != self.container.oid:
                        ...
                        self.task.progress(self.step_id, msg='clone openstack volume %s to %s' %
                                                             (volume.ext_id, clone_volume_id))
                        ...
                        while cond is True:
                            ...
                            sleep(2)
                        self.task.progress(self.step_id, msg='retype openstack volume %s' % clone_volume_id)

                        # manage volume in server container
                        final_volume = self.container.conn.volume_v3.manage(clone_volume_id, name, volume_type.ext_id,
                                                                            bootable=boot, desc=desc,
                                                                            availability_zone='nova', host=backend_host)
                        self.task.progress(self.step_id, msg='manage openstack volume %s on container %s' %
                                                             (clone_volume_id, self.container.oid))

                        # unmanage volume in original container
                        orig_container.conn.volume_v3.unmanage(clone_volume_id)
                        self.task.progress(self.step_id, msg='manage openstack volume %s on container %s' %
                                                             (clone_volume_id, orig_container.oid))

                        # attach remote volume
                        self.container.update_resource(final_volume_resource.id, ext_id=final_volume['id'])
                        self.task.progress(self.step_id, msg='attach openstack volume %s to volume %s' %
                    ...
                    # link volume id to server
                    server.add_link('%s-%s-volume-link' % (server.oid, volume.oid), 'volume', volume.oid,
                                    attributes={'boot': boot})
                    self.task.progress(self.step_id, msg='Setup volume link from %s to server %s' %
                                                         (volume.oid, server.oid))
    ...
    def delete_physical_server(self, server_ext_id):
        ...
        self.task.progress(self.step_id, msg='Get server %s volumes: %s' % (server_ext_id, truncate(volumes, 200)))
        ...
        while True:
            try:
                inst = OpenstackServer.get_remote_server(resource.controller, server_ext_id, self.container,
                                                         server_ext_id)
                if inst == {}:
    ...
    def delete_physical_port(self, port_ext_id):
        ...
        while True:
            try:
                inst = OpenstackPort.get_remote_port(self.container.controller, port_ext_id, self.container,
                                                     port_ext_id)
                status = inst['status']
                if status == 'ERROR':
    ...
    def delete_physical_volume(self, volume_ext_id):
        ...
        for snapshot in snapshots:
            ...
            while True:
                try:
                    ...
                    self.task.progress(self.step_id, msg='Volume %s snapshot %s deleted' %
                                                         (volume_ext_id, snapshot['id']))
                    break
        ...
        while True:
            try:
                inst = OpenstackVolume.get_remote_volume(self.container.controller, volume_ext_id, self.container, 
                                                         volume_ext_id)
                status = inst['status']
...
class ServerTask(AbstractResourceTask):
    ...
    def server_create_physical_step(task, step_id, params, *args, **kvargs):
        ...
        boot_volume_id = helper.create_volume(volume_name, volume_desc, parent_id, project_extid, availability_zone,
                                              main_volume, resource, boot=True)
        ...
        for network in networks:
            ...
            if subnet is not None and uuid is not None:
                ...
                port = conn.network.port.create(port_name, uuid, [fixed_ip], tenant_id=project_extid,
        ...
        server = conn.server.create(name, flavor, accessipv4=None, accessipv6=None, networks=nets,
                                    boot_volume_id=boot_volume_id, adminpass=admin_pass, description=desc,
                                    metadata=None, image=None, security_groups=sgs, personality=None,
                                    user_data=user_data, availability_zone=availability_zone, config_drive=config_drive)
        server_id = server['id']
        ...
        for config in volumes:
            ...
            volume_id = helper.create_volume(volume_name, volume_desc, parent_id, project_extid, availability_zone,
                                             config, resource)
    ...
    def server_expunge_physical_step(task, step_id, params, *args, **kvargs):
        ...
        # remove volume group
        resource.delete_volume_group_for_snapshot()
        task.progress(step_id, msg='Remove volume group used for snapshot')
    ...
    @staticmethod
    def server_action(task, step_id, action, success, error, params, final_status='ACTIVE', projectid=None):
    ...
    def server_start_step(task, step_id, params, *args, **kvargs):
        ...
        res = ServerTask.server_action(task, step_id, start_action, 'Start server', 'Error starting server',
                                       params, final_status='ACTIVE')
        return res, params
    ...
    def server_migrate_step(task, step_id, params, *args, **kvargs):
        ...
        res = ServerTask.server_action(task, step_id, migrate_action, 'Migrate server', 'Error migrating server',
                                       params, final_status='ACTIVE')
        return res, params
    ...
    def server_set_flavor_step(task, step_id, params, *args, **kvargs):
        ...
        res = ServerTask.server_action(task, step_id, set_flavor_action, 'Set flavor to server',
                                       'Error setting flavor to server', params, final_status='ORIGINAL')
        return res, params
    ...
    def server_add_volume_step(task, step_id, params, *args, **kvargs):
        ...
        def add_volume_action(container, resource, **params):
            ...
            server_obj.add_link('%s-%s-volume-link' % (volume_obj.oid, volume_obj.oid), 'volume', volume_obj.oid,
        ...
        res = ServerTask.server_action(task, step_id, add_volume_action, 'Attach volume to server',
                                       'Error attaching volume to server', params, final_status='ACTIVE')
        return res, params
    ...
    def server_del_volume_step(task, step_id, params, *args, **kvargs):
        ...
        def del_volume_action(container, resource, **params):
            ...
            while True:
                ...
                if status == 'available':
                    ...
                    task.progress(step_id, msg='Detach openstack volume %s - Error' % volume_extid)
        ...
        res = ServerTask.server_action(task, step_id, del_volume_action, 'Detach volume from server',
                                       'Error detaching volume from server', params, final_status='ACTIVE')
        return res, params
    ...
    def server_add_security_group_step(task, step_id, params, *args, **kvargs):
        ...
        res = ServerTask.server_action(task, step_id, add_security_group_action, 'Add security group to server',
                                       'Error adding security_group to server', params, final_status='ORIGINAL')
        return res, params
    ...
    def server_del_security_group_step(task, step_id, params, *args, **kvargs):
        ...
        res = ServerTask.server_action(task, step_id, del_security_group_action, 'Detach security group from server',
                                       'Error detaching security_group from server', params, final_status='ORIGINAL')
        return res, params
    ...
    def server_add_snapshot_step(task, step_id, params, *args, **kvargs):
        ...
        def add_snapshot_action(container, resource, **params):
            conn = container.conn
            server_obj = params['server']
            volume_group_id = server_obj.get_volume_group_for_snapshot(conn)
            conn.volume_v3.group.create_snapshot(volume_group_id, name=params['snapshot'])

            # refresh snapshots list
            # OpenstackVolume.list_remote_volume_snapshots(container.controller, container.oid, container, None)
        ...
        res = ServerTask.server_action(task, step_id, add_snapshot_action, 'Add server snapshot',
                                       'Error adding snapshot to server', params, projectid=projectid,
    ...
    def server_del_snapshot_step(task, step_id, params, *args, **kvargs):
        ...
        def del_snapshot_action(container, resource, **params):
            conn = container.conn
            server_obj = params['server']
            conn.volume_v3.group.delete_snapshot(params['snapshot'])

            ServerTask.__wait_for_snapshot(conn, params['snapshot'])

            # refresh snapshots list
            # OpenstackVolume.list_remote_volume_snapshots(container.controller, container.oid, container, None)

            # get snapshot list
            volume_group_ext_id = server_obj.get_attribs('volume_group_ext_id')
            snapshots = conn.volume_v3.group.list_snapshots(group_id=volume_group_ext_id, detail=False)
            # if no snapshots exists remove volume group
            if len(snapshots) == 0:
                server_obj.set_container(container)
                server_obj.delete_volume_group_for_snapshot()
        ...
        res = ServerTask.server_action(task, step_id, del_snapshot_action, 'Delete server snapshot',
                                       'Error removing snapshot from server', params, projectid=projectid,
                                       final_status='ORIGINAL')
        return res, params
    ...
    def server_revert_snapshot_step(task, step_id, params, *args, **kvargs):
        ...
        def revert_snapshot_action(container, resource, **params):
            ...
            server_obj.revert_to_volume_group_snapshot(conn, params['snapshot'])
        ...
        res = ServerTask.server_action(task, step_id, revert_snapshot_action, 'Revert server to snapshot',
                                       'Error when revert server to snapshot', params, projectid=projectid)
        return res, params
    ...
    def server_restore_from_backup(task, step_id, params, *args, **kvargs):
        ...
        def restore_from_backup(container, resource, **params):
            ...
            # delete snapshot
            restore = trilio_conn.restore.server(params['restore_point'], server_obj.ext_id,
                                                 server_name=server_name, target_network=None,
                                                 target_subnet=None, keep_original_ip=False, overwrite=False,
                                                 new_token=None)
            restore_id = restore['id']
            task.progress(step_id, msg='restore server %s with restore: %s - START' % (server_obj.ext_id, restore_id))
            ...
            model = container.add_resource(objid=objid, name=server_name, resource_class=OpenstackServer,
                                           ext_id=server_ext_id, active=True, desc=server_name,
                                           attrib={}, parent=project_id)
            params.update({'id': model.id, 'uuid': model.uuid})
            server = OpenstackServer(container.controller, oid=model.id, objid=objid, name=server_name,
                                     desc=server_name, active=True, model=model)
            ...
            for volume in inst.get('os-extended-volumes:volumes_attached', []):
                ...
                volume_ext_obj = OpenstackVolume.get_remote_volume(container.controller, volume_ext_id, container,
                                                                   volume_ext_id)
                volume_name = volume_ext_obj['name']
                volume_boot = str2bool(volume_ext_obj['bootable'])
                objid = '%s//%s' % (project.objid, id_gen())
                model = container.add_resource(objid=objid, name=volume_name, resource_class=OpenstackVolume,
                                               ext_id=volume_ext_id, active=True, desc=volume_name,
                                               attrib={}, parent=project_id)
                volume = OpenstackVolume(container.controller, oid=model.id, objid=objid, name=volume_name,
                                         desc=volume_name, active=True, model=model)
                volume.update_state(ResourceState.ACTIVE)
                server.add_link('%s-%s-volume-link' % (server.oid, model.id), 'volume', model.id,
                                attributes={'boot': volume_boot})
                task.progress(step_id, msg='Setup volume link from %s to server %s' % (model.id, server.oid))

            task.progress(step_id, msg='restore server %s with restore: %s - STOP' % (server_obj.ext_id, restore_id))
        ...
        params['server'] = server
        projectid = parent.oid
        res = ServerTask.server_action(task, step_id, restore_from_backup,
                                       'Restore server from backup',
