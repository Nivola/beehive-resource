...
class ProjectTask(AbstractResourceTask):
    ...
    def project_create_physical_step(task, step_id, params, *args, **kvargs):
        ...
        inst = conn.project.create(
            name,
            domain_name,
            is_domain=is_domain,
            parent_id=parent_extid,
        ...
        task.progress(
            step_id,
            msg="Assign trilio_backup_role role for project %s to admin user" % inst["id"],
        )
    ...
    def project_register_securitygroup_step(task, step_id, params, *args, **kvargs):
        ...
        model = container.add_resource(
            objid=default_sg_objid,
            name=sg["name"],
            resource_class=OpenstackSecurityGroup,
            ext_id=sg["id"],
            active=True,
            desc=desc,
            attrib=None,
        ...
        for sg in sgs:
            ...
            if task.is_ext_id_valid(sg.ext_id) is True:
                try:
                    ...
                    task.progress(
                        step_id,
                        msg="Security group %s does not already exist" % sg.ext_id,
                    )
                    return None
