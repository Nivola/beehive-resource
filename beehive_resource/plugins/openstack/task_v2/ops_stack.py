...
from beehive_resource.plugins.openstack.entity.ops_heat import OpenstackHeatStack, stack_entity_type_mapping
...
class StackTask(AbstractResourceTask):
    ...
    def stack_create_physical_step(task, step_id, params, *args, **kvargs):
        ...
        # create new stack
        stack = conn.heat.stack.create(stack_name=name, template=template, environment=environment,
                                       parameters=parameters, tags=tags, files=files, stack_owner=stack_owner)
        stack_id = stack['id']
    ...
    def stack_delete_physical_step(task, step_id, params, *args, **kvargs):
        ...
        if task.is_ext_id_valid(ext_id) is True:
            ...
            if inst['stack_status'] != 'DELETE_COMPLETE':
                ...
                while True:
                    inst = OpenstackHeatStack.get_remote_stack(container.controller, ext_id, container, res.name,
                                                               ext_id)
                    status = inst.get('stack_status', None)
                    if status == 'DELETE_COMPLETE':
                        break
                    elif status == 'DELETE_FAILED':
                        err = 'Delete stack %s - Error: %s' % (ext_id, inst.get('stack_status_reason', ''))
