...
class PortTask(AbstractResourceTask):
    ...
    def port_create_physical_step(task, step_id, params, *args, **kvargs):
        ...
        conn = container.conn
        inst = conn.network.port.create(name, network, fixed_ips, host_id, profile, vnic_type, device_owner,
    ...
    def port_del_security_group_step(task, step_id, params, *args, **kvargs):
        ...
        task.progress(step_id, msg='Remove security group %s from port %s' % (security_group, ext_id))
