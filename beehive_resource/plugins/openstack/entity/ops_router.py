...
class OpenstackRouter(OpenstackResource):
    ...
    def discover_new(container, ext_id, res_ext_ids):
        ...
        for item in items:
            if item['id'] not in res_ext_ids:
                ...
                res.append((OpenstackRouter, item['id'], parent_id, OpenstackRouter.objdef, name, level))
    ...
    def pre_create(controller, container, *args, **kvargs):
        ...
        params = {
            'desc': 'Router %s' % kvargs['name'],
            'project_extid': project.ext_id
        }
            ...
            for port in ports:
                ...
                if device_owner == 'network:router_ha_interface':
                    if self.container.get_resource_by_extid(ext_id) is None:
                        ...
                        model = self.add_resource(objid=objid, name=name, resource_class=OpenstackNetwork,
                                                  ext_id=ext_id, active=True, desc=desc, attrib={}, parent=None)
    ...
    def reset_routes(self):
        ...
        if self.ext_id is not None:
            self.clean_cache()
            res = self.container.conn.network.router.update(self.ext_id, routes=[])
            return res
        return False
