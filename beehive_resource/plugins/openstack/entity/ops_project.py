...
from beehive_resource.plugins.openstack.entity.ops_heat import OpenstackHeatStack,\
    OpenstackHeatTemplate, OpenstackHeatSWconfig, OpenstackHeatSWdeployment
...
class OpenstackProject(OpenstackResource):
    ...
    def discover_new(container, ext_id, res_ext_ids):
        ...
        for item in items:
            if item['id'] not in res_ext_ids:
                ...
                res.append((OpenstackProject, item['id'], parent_id, OpenstackProject.objdef, name, level))
    ...
    def pre_delete(self, *args, **kvargs):
        ...
        """
        kvargs['sgs'] = []
        sgs, total = self.container.get_resources(parent=self.oid, run_customize=False,
    ...
    def set_quotas(self, quotas):
        ...
        try:
            for quota in quotas:
                res = self.container.conn.project.update_quota(self.ext_id,
                                                               quota.get('type'),
                                                               quota.get('quota'),
                                                               quota.get('value'))
        except Exception as ex:
    ...
    def get_security_groups(self):
        ...
        sgs, total = self.controller.get_resources(
            parent=self.oid, type=OpenstackSecurityGroup.objdef, objdef=OpenstackSecurityGroup.objdef,
    ...
    @trace(op='update')
    def create_backup_job(self, name, instances, metadata={}, desc=None, fullbackup_interval=2,
                          start_date=None, end_date=None, start_time='0:00 AM', interval='24hrs', snapshots_to_retain=4,
                          timezone='Europe/Rome', job_type='Parallel'):
        ...
        try:
            if trilio_conn is not None:
                ...
                res = trilio_conn.workload.add(name, workload_type_id, instances, metadata=metadata, desc=desc,
    ...
    @trace(op='update')
    def update_backup_job(self, job_id, name=None, instances=None, metadata=None, desc=None, fullbackup_interval=None,
                          start_date=None, end_date=None, start_time=None, interval=None, snapshots_to_retain=None,
                          timezone=None, enabled=None):
        ...
        try:
            if trilio_conn is not None:
                ...
                trilio_conn.workload.update(job_id, name=name, instances=instances, metadata=metadata, desc=desc,
                                            fullbackup_interval=fullbackup_interval, start_date=start_date,
                                            end_date=end_date, start_time=start_time, interval=interval,
    ...
    def add_backup_restore_point(self, *args, **kvargs):
        ...
        res = self.action('add_backup_restore_point', steps, log='Add backup restore point', check=check,
                          **kvargs)
        return res
    ...
    def del_backup_restore_point(self, *args, **kvargs):
        ...
        res = self.action('del_backup_restore_point', steps, log='Delete backup restore point', check=check,
                          **kvargs)
        return res
