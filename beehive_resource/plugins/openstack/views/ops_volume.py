...
class CreateVolumeParamRequestSchema(Schema):
    ...
    source_volid = fields.String(required=False, missing=None, description='The UUID of the source volume. The API '
                                 'creates a new volume with the same size as the source volume.')
    snapshot_id = fields.String(required=False, missing=None, description='To create a volume from an existing '
    ...
    imageRef = fields.String(required=False, missing=None, description='The UUID of the image from which you want to '
                             'create the volume. Required to create a bootable volume.')
    volume_type = fields.String(required=True, example=None, description='The volume type. To create an environment '
                                'with multiple-storage back ends, you must specify a volume type.')
    metadata = fields.Dict(required=False, missing=None, description='One or more metadata key and value pairs that '
...
class CloneVolumeParamRequestSchema(Schema):
    ...
    project = fields.String(required=True, default='test', example='test', description='cloned volume project id')
...
class GetVolumeMetadata(OpenstackVolumeApiView):
    ...
    responses = SwaggerApiView.setResponses({
        200: {
            'description': 'success',
            'schema': GetVolumeMetadataResponseSchema
        }
    })
...
class GetVolumeSnapshot(OpenstackVolumeApiView):
    ...
    responses = SwaggerApiView.setResponses({
        200: {
            'description': 'success',
            'schema': GetVolumeSnapshotResponseSchema
        }
    })
