...
class ListNetworksRequestSchema(ListResourcesRequestSchema):
    segmentation_id = fields.Integer(example=345, context='query', description='[optional] An isolated segment on the '
    ...
    shared = fields.Boolean(context='query', example=False, description='Indicates whether this network is shared '
                            'across all tenants. By default, only administrative users can change this value')
    external = fields.Boolean(context='query', example=False, description='Indicates whether this network is '
    ...
    def validate_networks(self, data, *args, **kvargs):
        ...
        if 'segmentation_id' in keys or 'shared' in keys or 'external' in keys:
            if 'container' not in keys:
                raise ValidationError('container is required when segmentation_id, shared or esternal are used as '
                                      'filter')
...
class ListNetworks(OpenstackNetworkApiView):
    ...
    responses = SwaggerApiView.setResponses({
        200: {
            'description': 'success',
            'schema': ListNetworksResponseSchema
        }
    })
...
class GetNetwork(OpenstackNetworkApiView):
    ...
    responses = SwaggerApiView.setResponses({
        200: {
            'description': 'success',
            'schema': GetNetworkResponseSchema
        }
    })
...
class CreateNetworkParamRequestSchema(Schema):
    ...
    shared = fields.Boolean(default=False, example=False, description='Indicates whether this network is shared across '
                            'all tenants. By default, only administrative users can change this value')
    external = fields.Boolean(default=False, example=False, description='Indicates whether this network is externally '
                                                                        'accessible')
    project = fields.String(default='12', example='12', required=True, description='The id or uuid of the project that '
                                                                                   'owns the network')
    qos_policy_id = fields.UUID(allow_none=True, example='4cdf0ea4-159a-45aa-96f2-708e461130e1',
    ...
    physical_network = fields.String(default='vs1', example='vs1', required=False,
    ...
    network_type = fields.String(default='vxlan', example='vlan', description='[default=vlan] The type of physical '
                                 'network that maps to this network resource. For example, flat, vlan, vxlan, or gre',
                                 validate=OneOf(['vlan', 'vxlan', 'gre', 'flat']))
    segmentation_id = fields.Integer(example=345, allow_none=True, description='[optional] An isolated segment on he '
