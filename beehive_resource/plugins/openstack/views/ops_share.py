...
class ListShareTypesParamsResponseSchema(ResourceResponseSchema):
    ...
    id = fields.String(
        required=True,
        example="103c9cb9-00cf-4cf5-a93f-4bd4101dbfaf",
        description="share type id",
    )
...
class CreateShareParamRequestSchema(Schema):
    ...
    share_proto = fields.String(
        required=True,
        example="NFS",
        description="The Shared File Systems protocol. A valid "
    ...
    size = fields.Integer(
        required=True,
        example=23,
        description="The share size, in GBs. The requested share size "
        "cannot be greater than the allowed GB quota. To view the allowed quota, issue a get limits "
        "request.",
    )
    share_type = fields.String(
        required=True,
        example="netapp-nfs-565",
        description="The share type name. If you "
...
class GetShareGrantParamsResponseSchema(Schema):
    access_level = fields.String(
        required=True,
        example="rw",
        description="The access level to the share. To grant "
    ...
    state = fields.String(
        required=True,
        example="active",
        description="The state of access rule of a given share. " "This could be new, active or error.",
    )
    id = fields.String(
        required=True,
...
class GetShareNetworkResponseSchema(Schema):
    ...
    neutron_subnet_id = fields.String(
        required=True,
        exmaple="4dbd4f6c-aefe-4a36-9c67-050f96857f33",
    ...
    project_id = fields.String(
        required=True,
        exmaple="1a09ddd80cc24e19bff8d297cf7b2773",
...
class GetShareNetworks(OpenstackShareApiView):
    ...
    responses = SwaggerApiView.setResponses({200: {"description": "success", "schema": GetShareNetworksResponseSchema}})
...
class CreateShareNetworkParamRequestSchema(Schema):
    ...
    network = fields.String(
        required=True,
        exmaple="test",
        description="The UUID of a neutron network when setting up "
    ...
    subnet = fields.String(
        required=True,
        exmaple="test",
        description="The UUID of the neutron subnet when setting up "
        "or updating a share network subnet with neutron. Specify both a neutron network and a "
        "neutron subnet that belongs to that neutron network",
    )
    availability_zone = fields.String(
        ...
        exmaple="test",
        missing="nova",
        description="The UUID or name of " "an availability zone for the share network subnet",
    )
...
class CreateShareNetwork(OpenstackShareApiView):
    ...
    responses = SwaggerApiView.setResponses(
        {200: {"description": "success", "schema": CreateShareNetworkResponseSchema}}
    )
...
class DeleteShareNetwork(OpenstackShareApiView):
    ...
    responses = SwaggerApiView.setResponses(
        {202: {"description": "success", "schema": DeleteShareNetworkResponseSchema}}
    )
