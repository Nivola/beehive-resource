...
class VsphereContainer(Orchestrator):
    ...
    **connection syntax**:
    ...
    def ping(self):
        ...
        """
        try:
            self._get_connection(timeout=30)
            res = self.conn.ping()
        except:
            res = False
    ...
    def detail(self):
        ...
        res = info
        res["details"] = {"nsx": {"enabled": self.nsx_enabled}}
    ...
    def query_remote_task(self, task, step_id, vsphere_task, waitfor=True, error=None, delta=2):
        ...
        try:
            ...
            if error is None:
                # fix 'AttributeError' object has no attribute 'value'
                # error = ex.value
                error = str(ex)
    ...
    def _print_system_tree_item(self, obj, objdef, parent, res_index):
        ...
        extid = obj._moId
        try:
            oid = res_index[extid].oid
        except:
            oid = randint(10000, 20000)
        res = {
            "id": oid,
            "extid": extid,
            "name": name,
    ...
    def get_system_tree(self, vm=True, net=True, store=True, host=True):
        ...
        try:
            ...
            for datacenter in datacenters:
                obj = {
                    "id": res_index[datacenter._moId].oid,
