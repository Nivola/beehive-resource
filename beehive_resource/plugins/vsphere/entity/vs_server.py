...
class VsphereServer(VsphereResource):
    ...
    def align_resource_volumes(self, **params):
        server = params["server"]
        old_physical_volumes = params["physical_volumes"]
        ext_id = params["ext_id"]
        platform_server = server.get_by_morid(ext_id)
        hw = server.hardware
        # Re-ask physical volumes
        new_physical_volumes = {v.diskObjectId: v for v in hw.get_hard_disks(platform_server)}
        for vol_oid in old_physical_volumes:
            if vol_oid not in new_physical_volumes:
                vol = old_physical_volumes[vol_oid]
                backing = vol.backing
                datastore = backing.datastore
                backing_filename = backing.fileName
                size = vol.capacityInKB / 1024 / 1024
                unit_number = vol.unitNumber
                disk_type = None
                if vol.backing.thinProvisioned:
                    disk_type = "thin"
                hw.add_hard_disk(
                    platform_server,
                    size,
    ...
    def do_patch(self, **params):
        ...
        for physical_volume in physical_volumes:
            ...
            disk_object_id = str(physical_volume.get("disk_object_id"))
            self.logger.debug("do_patch - physical_volume - name: %s" % name)
            self.logger.debug("do_patch - physical_volume - unit_number: %s" % unit_number)
            self.logger.debug("do_patch - physical_volume - disk_object_id: %s" % disk_object_id)
            ...
            linked_volume = (
                linked_volume_by_unit_number
                if linked_volume_by_unit_number is not None
    ...
    def detail(self):
        ...
        if self.ext_obj is not None:
            ...
            # get linked volumes
            volumes, tot = self.get_linked_resources(
            ...
            for volume in data["volumes"]:
                ...
                if v is not None:
                    ...
                    # Fallback: get id with the old positional unit number
    ...
    def get_flavor_resource(self):
        ...
        if self.ext_obj is not None:
            ...
            ress, tot = self.container.get_resources(
                json_attribute_contain=json_attribute_contain,
                size=-1,
                with_perm_tag=False,
                active=1,
            )
    ...
    def get_volumes(self):
        ...
        try:
            if self.ext_obj is not None:
                ...
                linkvols, tot = self.get_linked_resources(
                    link_type_filter="volume",
                    entity_class=VsphereVolume,
                    objdef=VsphereVolume.objdef,
                    run_customize=False,
    ...
    def get_storage(self):
        ...
        try:
            ...
            for item in vm.storage.perDatastoreUsage:
                ...
                storage = {
                    "committed": round(item.committed / 1073741824, 2),
    ...
    def extend_volume(self, *args, **kvargs):
        ...
        steps = ["beehive_resource.plugins.vsphere.task_v2.vs_server.ServerTask.server_extend_volume_step"]
