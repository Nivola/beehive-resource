...
class ListDvpgs(VsphereDvpgApiView):
    ...
    responses = SwaggerApiView.setResponses({
        200: {
            'description': 'success',
            'schema': ListDvpgsResponseSchema
        }
    })
...
class CreateDvpgParamRequestSchema(Schema):
    ...
    physical_network = fields.String(required=True, example='test',
                                     description='dvs id, uuid or name')
    network_type = fields.String(required=False, example='vlan', default='vlan',
                                 description='Only vlan is supported')
    segmentation_id = fields.Integer(required=True, example=567,
                                    description='Netwrok vlan id')
    numports = fields.Integer(required=False, example=24, default=24,
