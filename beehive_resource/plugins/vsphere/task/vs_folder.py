...
from beehive_resource.tasks import ResourceJobTask, ResourceJob,\
    expunge_resource_pre, expunge_resource_post,\
...
def vsphere_folder_create_entity(self, options):
    ...
    **Return:**
    ...
    # create vsphere folder
    inst = conn.folder.create(name, folder=folder, datacenter=dc, host=host, network=network, storage=storage, vm=vm,
                              desc=desc)
    inst_id = inst._moId
