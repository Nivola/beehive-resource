...
class VsphereServerHelper(object):
    ...
    def create_volume(self, name, desc, parent_id, server, volume_type, config, boot=False):
        ...
        if source_type == "image":
            ...
            volume = self.orchestrator.resource_factory(
                VsphereVolume,
                name=name,
                desc=desc,
                parent=parent_id,
    ...
    def clone_from_template(
        ...
        # reconfigure vm
        task = self.conn.server.reconfigure(
            inst,
            network.ext_obj,
            disks=disks,
            memoryMB=memory_mb,
        ...
        if self.is_windows(template.ext_obj) is True:
            ...
            task = self.conn.server.customize(
                inst,
    ...
    def linked_clone_from_server(
        ...
        # start creation
        task = self.conn.server.create_linked_clone(
            server.ext_obj,
            name,
            folder.ext_obj,
            main_datastore,
