    ...
    **Returns:**
    ...
    try:
        res = conn.network.nsx.dfw.create_rule(
            sectionid, name, action, direction=direction, logged=logged,
            sources=sources, destinations=destinations, services=services,
            appliedto=appliedto, precedence=precedence)
...
def nsx_dfw_rule_move_entity(self, options):
    ...
    **Returns:**
    ...
    # move nsx dfw rule
    res = conn.network.nsx.dfw.move_rule(sectionid, ruleid, 
                                         ruleafter=ruleafter)
...
def nsx_dfw_rule_update_entity(self, options):
    ...
    **Returns:**
    ...
    # update nsx dfw rule
    res = conn.network.nsx.dfw.update_rule(sectionid, ruleid, new_action=action,
                                           new_disable=disable, new_name=name)
