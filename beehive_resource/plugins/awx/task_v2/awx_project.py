...
class AwxProjectTask(AbstractResourceTask):
    ...
    def awx_project_create_physical_step(task, step_id, params, *args, **kvargs):
        ...
        conn = container.conn
        inst = conn.project.add(name, description=desc, organization=org_id, scm_type=scm_type, scm_url=scm_url,
                                scm_branch=scm_branch, scm_update_on_launch=scm_update_on_launch, credential=scm_creds)
        inst_id = inst['id']
    ...
    def awx_project_delete_physical_step(task, step_id, params, *args, **kvargs):
        ...
        if resource.is_ext_id_valid() is True:
            try:
                ...
                task.progress(step_id, msg='Awx project %s does not exist anymore' % resource.ext_id)
