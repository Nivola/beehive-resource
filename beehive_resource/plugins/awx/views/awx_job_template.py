...
from beehive_resource.plugins.provider.views import CreateProviderResourceRequestSchema, \
...
class ListAwxJobTemplatesResponseSchema(PaginatedResponseSchema):
    job_templates = fields.Nested(ListAwxJobTemplatesParamsResponseSchema, many=True, required=True, allow_none=True)
...
class ListAwxJobTemplates(AwxJobTemplateView):
    ...
    responses = SwaggerApiView.setResponses({
        200: {
            'description': 'success',
            'schema': ListAwxJobTemplatesResponseSchema
        }
    })
...
class GetAwxJobTemplate(AwxJobTemplateView):
    ...
    responses = SwaggerApiView.setResponses({
        200: {
            'description': 'success',
            'schema': GetAwxJobTemplateResponseSchema
        }
    })
...
class AwxJobTemplateHostRequestSchema(Schema):
    name = fields.String(required=True, example='test.nivolapiemonte.it', default='', description='Host IP or FQDN')
    extra_vars = fields.String(description='host variables. Ex: k1:v1;k2:v2', default='',
...
class CreateAwxJobTemplateAddParamRequestSchema(Schema):
    ...
    hosts = fields.Nested(AwxJobTemplateHostRequestSchema, required=False, many=True, allow_none=True,
                          description='Hosts to add to inventory')
    project = fields.String(required=True, example='test-project', default='', description='Awx project name')
    playbook = fields.String(required=True, example='main.yml', default='', description='Playbook')
    verbosity = fields.Integer(example=1, default=0, description='Verbosity: 0 (Normal) (default), 1 (Verbose), '
...
class CreateAwxJobTemplateLaunchParamRequestSchema(Schema):
    ssh_creds = fields.Nested(AwxJobTemplateHostSshCredsRequestSchema, required=False, description='Ssh credentials')
    ssh_cred_id = fields.String(required=False, description='Ssh credential id')
    extra_vars = fields.String(description='Variables used when launching job template. Ex: k1:v1;k2:v2', default='',
...
class CreateAwxJobTemplateParamRequestSchema(CreateProviderResourceRequestSchema):
    ...
    name = fields.String(required=True, example='test-job-template', default='', description='Job template name')
    desc = fields.String(example='test-job-template', default='', description='Job template description')
    add = fields.Nested(CreateAwxJobTemplateAddParamRequestSchema, required=True,
                        description='List of parameters to create an awx job template')
    launch = fields.Nested(CreateAwxJobTemplateLaunchParamRequestSchema, required=True,
...
class UpdateAwxJobTemplateTemplateRequestSchema(Schema):
    name = fields.String(required=True, example='test-job_template', default='', description='Job_template name')
...
class UpdateAwxJobTemplateParamRequestSchema(UpdateProviderResourceRequestSchema):
    job_templates = fields.Nested(UpdateAwxJobTemplateTemplateRequestSchema, required=False, many=True,
                                  description='list of orchestrator job_templates to link', allow_none=True)
