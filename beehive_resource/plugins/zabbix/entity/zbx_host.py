...
class ZabbixHost(ZabbixResource):
    ...
    def discover_died(container):
        ...
        for host in hosts:
            items.append({"id": host["hostid"], "name": host["name"]})
        return items
    ...
    def post_get(self):
        ...
        try:
            ...
            ext_groups = zabbixManager.host.groups(ext_obj["hostid"]).get("groups", [])
            for item in ext_groups:
                # group = resourceController.get_resource_by_extid(item["groupid"])
                res_group, total = zabbixContainer.get_resources(
                    objdef=ZabbixHostgroup.objdef,
                    type=ZabbixHostgroup.objdef,
                    ext_id=item["groupid"],
                    run_customize=False,
                )
                if total > 0:
                    group: ZabbixHostgroup = res_group[0]
                    self.groups.append(group)

            # retrieve templates linked to the host
            ext_templates = zabbixManager.host.templates(ext_obj["hostid"]).get("parentTemplates", [])
            for item in ext_templates:
                # template = resourceController.get_resource_by_extid(item["templateid"])
                res_template, total = zabbixContainer.get_resources(
                    objdef=ZabbixTemplate.objdef,
                    type=ZabbixTemplate.objdef,
                    ext_id=item["templateid"],
                    run_customize=False,
                )
                if total > 0:
                    template: ZabbixTemplate = res_template[0]
    ...
    def pre_create(controller, container, *args, **kvargs):
        ...
        try:
            for item in groups:
                obj = zabbixContainer.get_resource(item, entity_class=ZabbixHostgroup)
                if obj.is_ext_id_valid() is True:
                    # check whether hostgroup exists on the platform
                    res = zabbixManager.hostgroup.get(obj.ext_id)
            ...
            for item in templates:
                ...
                if obj.is_ext_id_valid() is True:
                    ...
                    res = zabbixManager.template.get(obj.ext_id)
                    # if exists, add it to list of ext_ids
    ...
    def do_expunge(self, **params):
        ...
        try:
            ...
            zabbixContainer: ZabbixContainer = self.container
            zabbixManager: ZabbixManager = zabbixContainer.conn
            zabbixManager.host.get(self.ext_id)
            # delete host
            zabbixManager.host.delete(self.ext_id)
