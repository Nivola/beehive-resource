...
from beehive_resource.plugins.vsphere.entity.nsx_dfw import NsxDfwSection,\
    NsxDfwRule
...
class ProviderVsphere(object):
    ...
    def type_mapping(task, cid, mtype, mvalue):
        ...
        try:
            if mtype == 'Network':
                ...
                res = {'type': 'DistributedVirtualPortgroup',
                       'value': net.ext_id,
                       'name': None}
            elif mtype == 'Cidr':
                res = {'type': 'Ipv4Address',
                       'value': mvalue,
                       'name': None}
            elif mtype == 'Server':
                res = {'type': 'Ipv4Address',
    ...
    def create_folder(task, cid, resource, datacenter, folder):
        ...
        res = container.resource_factory(VsphereFolder, name=name, desc=resource.desc, active=False, attribute={},
                                         parent=None, tags='', folder_type='vm', datacenter=datacenter, folder=folder)
        jobid = res[0]['jobid']
    ...
    def create_security_group(task, cid, resource, parent):
        ...
        container = task.get_container(cid)
        res = container.resource_factory(NsxSecurityGroup, name=name, desc=resource.desc, active=False, attribute={},
                                         parent=None, tags='')
        # jobid = res[0]['jobid']
        # task.progress('Start folder creation with job: %s' % jobid)
    ...
    def create_section(task, cid, resource):
        ...
        resource_model = orchestrator.add_resource(objid=objid, name=name, resource_class=CustomResource,
                                                   ext_id=section_id, active=True, desc=resource.desc,
                                                   attrib=attribs, parent=None, tags=['vsphere'])
        ...
        # set up resource link
        task.get_session(reopen=True)
        resource.add_link('%s-%s-link' % (section_id, resource_id), 'relation', resource_id, attributes={})
    ...
    def create_network(task, cid, resource, network_type, name, vlan, external, private,
        ...
        try:
            ...
            if network_type in ['flat', 'vlan']:
                ...
                resource.add_link('%s.%s-link' % (resource.oid, net.oid), 'relation', net.oid, attributes={})
    ...
    def create_subnet(task, cid, resource, cidr, gateway, routes, allocation_pools, enable_dhcp, dns_nameservers):
        ...
        try:
            ...
            if len(dns_nameservers) == 2:
                ...
                dns2 = None
            prefix = cidr.split('/')[1]
            ippool_id = nsx_manager.add_ippool(name, prefix=prefix, gateway=gateway, dnssuffix=None, dns1=dns1,
        ...
        try:
            ...
            if source['type'] == destination['type'] and \
               source['value'] == destination['value']:
                ...
                policies.append(ProviderVsphere.create_nsx_rule(
                        task, resource, cid, section, name+'-out', 'out', sources, dests, port, appliedto))
                policies.append(ProviderVsphere.create_nsx_rule(
                        task, resource, cid, section, name+'-in', 'in', sources, dests, port, appliedto))
    ...
    @staticmethod
    def create_nsx_rule(task, resource, cid, section, name, direction, source, dest, service, appliedto, logged=True):
        """Create nsx rule
        ...
        resource_model = orchestrator.add_resource(objid=objid, name=name, resource_class=CustomResource,
                                                   ext_id=rule_id, active=True, desc=desc, attrib=attribs, parent=None,
                                                   tags=['vsphere'])
        resource_id = resource_model.id
        orchestrator.update_resource_state(resource_id, 2)
        orchestrator.activate_resource(resource_id)
    ...
    def create_image(task, cid, resource, image_id, template_pwd, guest_id):
        ...
        try:
            ...
            resource.add_link('%s.%s-link' % (resource.oid, image_id), 'relation', image_id,
                              attributes={'reuse': True, 'template_pwd': template_pwd, 'guest_id': guest_id})
    ...
    def remove_resource(task, provider, cid, childs):
        ...
        try:
            ...
            for child in childs:
                ...
                try:
                    if definition in [VsphereFolder.objdef, NsxSecurityGroup.objdef, VsphereDvpg.objdef,
                                      VsphereServer.objdef, NsxIpSet.objdef]:
                        res = child.expunge()
                        jobid = res[0]['jobid']
                        task.wait_for_job_complete(jobid)
                        ...
                        if sub_type == NsxDfwRule.objdef:
                            ...
                            if rule == {}:
                                task.progress('Rule %s:%s does not exist' %
                                                             (attribs['section'], attribs['id']))
                            else:
                                dfw.delete_rule({'sectionid': attribs['section'], 'ruleid': attribs['id']})
