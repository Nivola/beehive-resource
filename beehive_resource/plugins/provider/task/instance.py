    ...
    # - instance is a twin. Get fixed ip from main instance
        ...
        for network in params.get("networks"):
            ...
            if len(nets) < 1:
                ...
                self.progress(
                    "Vps %s does not have network in availability zone %s" % (network["vpc"], availability_zone_id)
                )
                return None
            else:
                nets = nets[0]
...
def task_import_main_server(self, options):
    ...
    if main is True:
        ...
        server_id = ProviderOrchestrator.get(orchestrator.get("type")).import_server(
            self, orchestrator, instance, params
        )
