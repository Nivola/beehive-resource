...
from beehive_resource.plugins.provider.task import ProviderOpenstack, ProviderVsphere, ProviderOrchestrator, \
...
def task_create_zone_stack(self, options, template, stack_id):
    ...
    job_id = res[0]['jobid']
    stack_id = res[0]['uuid']
    self.update('PROGRESS', msg='Create stack in availability zone %s - start job %s' %
                (availability_zone_id, job_id))

    # wait job complete
    res = self.wait_for_job_complete(job_id)
    self.update('PROGRESS', msg='Create stack %s in availability zone %s' % (stack_id, availability_zone_id))
...
def task_create_zone_stack_twins(self, options, availability_zone_id, stack_id):
    ...
    job_id = res[0]['jobid']
    stack_id = res[0]['uuid']
    self.update('PROGRESS', msg='Create stack twin in availability zone %s - start job %s' %
                (availability_zone_id, job_id))

    # wait job complete
    res = self.wait_for_job_complete(job_id)
    self.update('PROGRESS', msg='Create stack twin %s in availability zone %s' % (stack_id, availability_zone_id))
...
def task_register_dns_compute_stack(self, options):
    ...
    if resolve is True:
        try:
            ...
            self.update('PROGRESS', msg='Register stack %s in dns with records %s' % (oid, uuids))
...
def task_create_twins(self, options):
    ...
    for server_conf in server_confs:
        ...
        # exec task when stack creation skipped
            ...
            if orchestrator is not None:
                ProviderOpenstack.create_port(self, orchestrator['id'], stack, network_id, subnet_cidr, fixed_ip,
                                              rule_groups)
