...
def task_create_zone_customization(self, options, avz_id, main):
    ...
    self.update('PROGRESS', msg='Create customization in availability zone %s - start job %s' % (avz_id, job_id))

    # wait job complete
    self.wait_for_job_complete(job_id)
    self.update('PROGRESS', msg='Create customization %s in availability zone %s' % (customization_id, avz_id))
...
def task_update_zone_customization(self, options, zone_customization_id, main):
    ...
    if size is not None:
        ...
        job_id = res[0]['jobid']
        self.update('PROGRESS', msg='Update customization size in availability zone %s - start job %s' % (oid, job_id))
    
        # wait job complete
        self.wait_for_job_complete(job_id)
        self.update('PROGRESS', msg='Update customization %s size in availability zone %s' % (oid, avz_id))
    ...
    if grant is not None:
        ...
        job_id = res[0]['jobid']
        self.update('PROGRESS', msg='Update customization grant in availability zone %s - start job %s' % (oid, job_id))

        # wait job complete
        self.wait_for_job_complete(job_id)
        self.update('PROGRESS', msg='Update customization %s grant in availability zone %s' % (oid, avz_id))
...
def task_link_customization(self, options, template_id):
    ...
    self.update('PROGRESS', msg='Link customization %s to compute customization %s' % (oid, compute_customization_id))
