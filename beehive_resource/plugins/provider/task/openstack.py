...
class ProviderOpenstack(object):
    QUOTAS = {
        ...
        'compute.volumes': {'type': 'blcreate_ruleock', 'quota': 'volumes', 'factor': 1},
    ...
    def create_project(task, cid, resource, domain, parent_project):
        ...
        res = container.resource_factory(OpenstackProject, name=name, desc=resource.desc, active=False, attribute={},
                                         parent=None, tags='', is_domain=False, enabled=True, domain_id=domain,
    ...
    def create_security_group(task, cid, resource, parent):
        ...
        try:
            ...
            container = task.get_container(cid)
            res = container.resource_factory(OpenstackSecurityGroup, name=name, desc=resource.desc, active=False,
    ...
    @staticmethod
    def create_network(task, cid, resource, network_type, name, vlan, external, private,
                       physical_network=None, public_network=None):
        ...
        try:
            ...
            networks, tot = container.get_resources(objdef=OpenstackNetwork.objdef, type=OpenstackNetwork.objdef,
            ...
            if tot > 0:
                ...
                # create link
                attrib = {'reuse': True}
                resource.add_link('%s.%s-link' % (resource.oid, network_id), 'relation', network_id,
                                  attributes=attrib)
    ...
    @staticmethod
    def create_subnet(task, cid, resource, cidr, gateway, routes, allocation_pools, enable_dhcp, dns_nameservers):
    ...
    def create_rule(task, cid, zone, resource, source, destination, service):
        ...
        try:
            ...
            # sgrule1 -> sgrule1
            if source['type'] == destination['type'] and \
               source['value'] == destination['value']:
                ...
                policies.append(ProviderOpenstack.create_openstack_rule(
                        task, resource, cid, sg_id, 'egress', 
                        port_range_min, port_range_max, 
                        proto, group=sg_id, cidr=None))
                policies.append(ProviderOpenstack.create_openstack_rule(
                        task, resource, cid, sg_id, 'ingress', 
                        port_range_min, port_range_max, 
                        proto, group=sg_id, cidr=None)) 
            ...
            # sgrule1 -> sgrule2
                ...
                policies.append(ProviderOpenstack.create_openstack_rule(
                        task, resource, cid, source_sg_id, 'egress', 
                        port_range_min, port_range_max, 
                        proto, group=dest_sg_id, cidr=None))
                policies.append(ProviderOpenstack.create_openstack_rule(
                        task, resource, cid, dest_sg_id, 'ingress', 
                        port_range_min, port_range_max, 
            ...
            # server -> sgrule
            elif destination['type'] in ['Environment'] and \
                 source['type'] in ['Server']:
    ...
    @staticmethod
    def create_openstack_rule(task, resource, cid, sg_id, direction, port_range_min, port_range_max, protocol,
                              group=None, cidr=None):
        ...
        objid = u"%s//%s" % (orchestrator.objid, id_gen())
        ...
        resource_model = orchestrator.add_resource(objid=objid, name=name, resource_class=CustomResource, ext_id=None,
        ...
        try:
            ...
            for network_conf in networks:
                ...
                if net == 0:
                    ...
                    user_data = ProviderOpenstack.user_data(gateway=subnet['gateway'], users=users,
                                                            pwd=admin_pass, sshkey=sshkey,
    ...
    def import_share(task, resource, share_id):
        ...
        try:
            # set up resource link
            task.get_session(reopen=True)
            resource.add_link('%s.%s-link' % (resource.oid, share_id), 'relation', share_id,
    ...
    def create_stack(task, orchestrator, resource, project, params, compute_stack_jobid):
        ...
        try:
            ...
            for server in servers:
                ...
                provider_nets = task.get_linked_resources(server_net['net_id'], link_type='relation',
                                                          container_id=None, objdef=None)
                # provider_net = provider_nets[0]
                provider_vpc = task.get_linked_resources(provider_nets[0].id, link_type='relation.%',
                                                         container_id=None, objdef=None)
                ...
                for server_sgs in server_sgss:
                    rg = task.get_linked_resources(server_sgs['id'], link_type='relation', container_id=None,
                                                   objdef=None)
                    if len(rg) > 0:
                        sg = task.get_linked_resources(rg[0].id, link_type='relation.%', container_id=None,
                                                       objdef=None)
            ...
            task.progress('Update compute stack job %s shared data with server confs: %s' %
    ...
    def import_flavor(task, cid, resource, flavor_id):
        ...
        try:
            # set up resource link
            task.get_session(reopen=True)
            resource.add_link('%s.%s-link' % (resource.oid, flavor_id), 'relation', flavor_id,
    ...
    def import_volumetype(task, cid, resource, volumetype_id):
        ...
        try:
            # set up resource link
            task.get_session(reopen=True)
            resource.add_link('%s.%s-link' % (resource.oid, volumetype_id), 'relation', volumetype_id,
                              attributes={'reuse': True})
    ...
    def create_volume(task, orchestrator, compute_volume, resource, project, params):
        ...
        try:
            ...
            resource.add_link('%s.%s-link' % (resource.oid, volume.oid), 'relation', volume.oid, attributes={})
    ...
    def import_volume(task, resource, volume_id):
        ...
        try:
            # set up resource link
            task.get_session(reopen=True)
            resource.add_link('%s.%s-link' % (resource.oid, volume_id), 'relation', volume_id,
                              attributes={'reuse': False})
    ...
    def create_image(task, cid, resource, image_id, *args):
        ...
        try:
            # set up resource link
            task.get_session(reopen=True)
            resource.add_link('%s.%s-link' % (resource.oid, image_id), 'relation', image_id,
                              attributes={'reuse': True})
    ...
    def server_action(task, cid, resource, action, params):
        ...
        try:
            ...
            jobid = res[0]['jobid']
            task.progress('Start run action %s over server %s with job: %s' %
                                         (action, remote_server.uuid, jobid))
    ...
    def remove_resource(task, provider, cid, childs):
        ...
        try:
            ...
            for child in childs:
                ...
                child = entity_class(container.controller, oid=child.id, objid=child.objid, name=child.name,
