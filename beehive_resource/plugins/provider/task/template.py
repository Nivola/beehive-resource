...
def task_create_zone_template(self, options, avz_id):
    ...
    self.update('PROGRESS', msg='Create template in availability zone %s - start job %s' % (avz_id, job_id))

    # wait job complete
    self.wait_for_job_complete(job_id)
    self.update('PROGRESS', msg='Create template %s in availability zone %s' % (template_id, avz_id))
...
def task_update_zone_template(self, options, zone_template_id):
    ...
    if size is not None:
        ...
        job_id = res[0]['jobid']
        self.update('PROGRESS', msg='Update template size in availability zone %s - start job %s' % (oid, job_id))
    
        # wait job complete
        self.wait_for_job_complete(job_id)
        self.update('PROGRESS', msg='Update template %s size in availability zone %s' % (oid, avz_id))
    ...
    if grant is not None:
        ...
        job_id = res[0]['jobid']
        self.update('PROGRESS', msg='Update template grant in availability zone %s - start job %s' % (oid, job_id))

        # wait job complete
        self.wait_for_job_complete(job_id)
        self.update('PROGRESS', msg='Update template %s grant in availability zone %s' % (oid, avz_id))
...
def task_link_template(self, options):
    ...
    self.update('PROGRESS', msg='Link template %s to compute template %s' % (oid, compute_template_id))
