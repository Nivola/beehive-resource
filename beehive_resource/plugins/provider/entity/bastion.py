...
class ComputeBastion(ComputeInstance):
    ...
    def get_bastion_security_group(self):
        ...
        sgs, tot = self.container.get_resources(name=sg_name, authorize=False,
    ...
    def manage(self, user=None, key=None, password='', *args, **kvargs):
        ...
        try:
            res = self.api_client.get_ssh_node(self.fqdn)
            uuid = res.get('uuid')
            self.logger.warning('Compute instance %s is already managed by ssh module' % self.uuid, exc_info=1)
            return uuid
        except BeehiveApiClientError as ex:
            if ex.code == 404:
                pass
    ...
    def unmanage(self):
        ...
        try:
            ...
            if ex.code == 404:
                self.logger.warning('Compute instance %s is not managed by ssh module' % self.uuid, exc_info=1)
            else:
                raise
