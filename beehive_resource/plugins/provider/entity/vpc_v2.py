...
class Vpc(ComputeProviderResource):
    ...
    def get_proxies(self, site_id):
        ...
        if self.is_shared() is True:
            ...
            vpc_nets, total = self.get_linked_resources(link_type='relation.%s' % site_id, objdef=SiteNetwork.objdef,
                                                        authorize=False, run_customize=False)
            vpc_net = vpc_nets[0]
            ...
            if bastion_host is not None:
                res['http'] = ('', '')
                res['zabbix'] = (bastion_host.get_ip_address(), bastion_host.get_zabbix_proxy_name())
        ...
        if site is not None:
            for item in site:
                ...
                if self.has_network(network_id) is False:
                    steps.append({'step': self.task_path + 'vpc_assign_network_step', 'args': [network_id]})
            ...
            for network in private:
                ...
                if self.get_private_network_by_cidr(cidr) is None:
                    steps.append({'step': self.task_path + 'vpc_add_network_step', 'args': [network]})
        ...
        if site is not None:
            for item in site:
                ...
                if self.has_network(network_id) is True:
                    steps.append({'step': self.task_path + 'vpc_deassign_network_step', 'args': [network_id]})
        elif private is not None:
            for data in private:
                cidr = data.get('cidr')
                ...
                steps.append({'step': self.task_path + 'vpc_del_network_step', 'args': [network.oid]})
...
class SiteNetwork(SiteChildResource):
    ...
    def pre_create(controller, container, *args, **kvargs):
        ...
        for item in orchestrator_idx.values():
            steps.append({'step': SiteNetwork.task_path + 'create_site_network_step', 'args': [item]})
    ...
    def get_zabbix_proxy(self):
        ...
        if zbx_proxy is None:
            raise ApiManagerError('No zabbix proxy available in attributes of network %s' % self.uuid, code=404)
    ...
    def add_subnets(self, params):
        ...
        res = Resource.action(self, 'add_subnet', steps, log='Add subnets to site network %s' % self.uuid, **params)
        return res
    ...
    def delete_subnets(self, params):
        ...
        res = Resource.action(self, 'del_subnet', steps, log='del subnets from site network %s' % self.uuid, **params)
        return res
    ...
    def pre_create(controller, container, *args, **kvargs):
        ...
        for orchestrator in orchestrator_idx.values():
            if index >= len(available_network_subnets):
                raise ApiManagerError('no available sub cidr found for orchestrator %s in base cidr %s' %
                                      (orchestrator['id'], base_cidr))
            ...
            steps.append({'step': PrivateNetwork.task_path + 'create_private_network_step',
