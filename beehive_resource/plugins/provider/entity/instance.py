...
class ComputeInstance(ComputeProviderResource):
    ...
    def get_flavor(self):
        ...
        if self.container is not None:
            remote_flavor = self.get_physical_server_param('get_flavor_resource')
            self.logger.warn('remote flavor: %s' % remote_flavor)
            flavor = self.container.get_simple_resource(remote_flavor.name)

        # res, total = self.get_linked_resources(link_type='flavor')
        # flavor = None
        # if total > 0:
        #     flavor = res[0]
    ...
    def get_real_admin_credential(self):
        ...
        res = {
            ...
            'ssh_key_data': credential.get('ssh_key_data', None),
    ...
    def __check_or_update_sg(self, update=False):
        # get sites
        availability_zones, tot = self.get_parent().get_linked_resources(link_type_filter='relation%',
                                                                         with_perm_tag=False, run_customize=False)
        ...
        for site in sites:
            res[site.name] = {}
            zone_instance, tot = self.get_linked_resources(link_type_filter='relation.%s' % site.oid,
                                                           with_perm_tag=False,
                                                           run_customize=False)
            if len(zone_instance) > 0:
                items, tot = zone_instance[0].get_linked_resources(link_type_filter='relation', with_perm_tag=False,
                                                                   run_customize=False)
                for item in items:
                    self.logger.warn('%s[%s]' % (item.objdef, item.ext_id))

                    for sg in sgs:
                        zone_sg, tot = sg.get_linked_resources(link_type_filter='relation.%s' % site.oid,
                                                               with_perm_tag=False,
                                                               run_customize=False)
                        ...
                        if sg_type == 'Vsphere.Nsx.NsxSecurityGroup':
                            run_customize = False
                        sgitems, tot = zone_sg[0].get_linked_resources(link_type_filter='relation', with_perm_tag=False,
                                                                       run_customize=run_customize, type=sg_type)
            ...
            for resource, zone_insts in res.items():
                for zone_inst in zone_insts:
                    if zone_inst.get_attribs().get('main', False) is True:
                        zone_instance = zone_inst

            if zone_instance is not None:
                self.physical_server = zone_instance.get_physical_server()
    ...
    def get_host(self):
        return self.get_physical_server().get_host()


    def get_host_group(self):
        return self.get_physical_server().get_host_group()
    ...
    def customize_list(controller, entities, *args, **kvargs):
        ...
        objdefs = [ComputeImage.objdef, SecurityGroup.objdef, Vpc.objdef, ComputeVolume.objdef]
    ...
    def pre_import(controller, container, *args, **kvargs):
        ...
        steps = [
            ...
            {'step': ComputeInstance.task_path + 'import_zone_instance_step', 'args': [main_availability_zone]},
    ...
    def pre_create(controller, container, *args, **kvargs):
        ...
        for block_device in block_devices:
            ...
            if source_type == 'image':
                ...
                imgs, tot = obj.get_linked_resources(link_type='relation.%s' % site.oid, run_customize=False,
                                                     objdef=Image.objdef)
                if tot == 0:
                    raise ApiManagerError('Image %s does not exist in availability zone %s' %
                                          (block_device['uuid'], site.uuid))
        ...
        for network in networks:
            # get vpc
            vpc = container.get_simple_resource(network['vpc'], entity_class=Vpc)
            vpc.check_active()
    ...
    def do_patch(self, **params):
        ...
        physical_volumes_missed = [pv for pv_id, pv in physical_volumes.items()
                                   if pv_id not in physical_volumes_present]
        ...
        for physical_volume in physical_volumes_missed:
            # bootable = physical_volume.is_bootable()
            ...
            self.add_link('%s-volume-%s-link' % (self.oid, volume.oid), 'volume.%s' % disk_index, volume.oid)
    ...
    def pre_delete(self, *args, **kvargs):
        ...
        # remove childs
        for child in childs:
            steps.append({'step': ComputeInstance.task_path + 'remove_child_step', 'args': [child]})
        # remove child volumes
        for child in child_volumes:
            steps.append({'step': ComputeInstance.task_path + 'remove_compute_volume_step', 'args': [child]})
            steps.append({'step': ComputeInstance.task_path + 'remove_child_step', 'args': [child]})
        # post expunge
    ...
    def manage(self, user=None, key=None, password='', *args, **kvargs):
        ...
        try:
            res = self.api_client.get_ssh_node(self.fqdn)
            uuid = res.get('uuid')
            self.logger.warning('Compute instance %s is already managed by ssh module' % self.uuid, exc_info=True)
            return uuid
        except BeehiveApiClientError as ex:
            if ex.code == 404:
                pass
        ...
        # create ssh node
        uuid = self.api_client.add_ssh_node(self.fqdn, self.desc, ip_address, group, user, key=key,
                                            attribute=jsonDumps(attribute), password=password)
    ...
    def unmanage(self):
        ...
        try:
            ...
            if ex.code == 404:
                self.logger.warning('Compute instance %s is not managed by ssh module' % self.uuid, exc_info=True)
            else:
                raise
    ...
    def get_physical_backup_status(self, cache=True, ttl=1800):
        ...
        if physical_server is not None:
            # TODO implemented for openstack vsphere to be done
            if hasattr(physical_server, "get_trilio_backup"):
                ...
                res = {'job': job, 'restore_points': restore_points}
            else:
                res = {'job': '', 'restore_points': restore_points}
        return res
    ...
    def is_image_opensource(self):
        if self.image is not None:
            config = self.image.get_configs()
            os = config.get('os').lower()
            if os.find('centos') >= 0 or os.find('ubuntu') >= 0 \
                    or config.get('os').find('OracleLinux') >= 0:
                return 1
        return 0
    ...
    def is_monitoring_enabled(self, cache=False, ttl=300):
        def func():
            ...
            res, total = self.controller.get_resources(objdef=ZabbixHost.objdef, name=self.fqdn.split('.')[0] + '%')
            self.logger.debug('is_monitoring_enabled - res: ' % res)
            if total == 1:
                return 1
    ...
    def restore_from_backup(self, restore_point=None, instance_name=None, *args, **kvargs):
        ...
        zone_instance = self.get_main_zone_instance()
        server_name = '%s-avz%s-%s-server' % (instance_name, zone_instance.get_parent().parent_id,
                                              zone_instance.get_physical_server().container_id)

        internal_steps = [
            {'step': ComputeInstance.task_path + 'send_action_to_zone_instance_step', 'args': [zone_instance.oid]},
            {'step': ComputeInstance.task_path + 'import_restored_server_step', 'args': []},
        ]
        res = {
            'internal_steps': internal_steps,
    ...
    def add_user(self, user_name=None, user_pwd=None, user_ssh_key=None, *args, **kvargs):
        ...
        internal_steps = [
            {'step': ComputeInstance.task_path + 'apply_customization_action_step', 'args': []},
    ...
    def disable_monitoring(self, *args, **kvargs):
        ...
        :return: kvargs
        """
        # todo: aggiornare il playbook zabbix per rimuovere l'agent. Adesso viene solo deregistrato su zabbix
        if self.is_windows() is True:
            # remove zabbix agent config from zabbix
            from beehive_resource.plugins.zabbix.entity.zbx_host import ZabbixHost
            res, total = self.controller.get_resources(objdef=ZabbixHost.objdef, name=self.fqdn.split('.')[0] + '%')
            if total == 0:
                self.logger.warn('+++++ ZabbixHost not found for %s' % self.fqdn)
            else:
                zabbixHost: ZabbixHost = res[0]
                zabbixHost.expunge2({}, sync=True)
                # res[0].expunge2({}, sync=True)
    ...
    def enable_logging(self, *args, **kvargs):
        ...
        internal_steps = [
            {'step': ComputeInstance.task_path + 'create_awx_job_template_step', 'args': job_template_args},
            ComputeInstance.task_path + 'apply_customization_action_step',
