...
from beehive_resource.plugins.provider.entity.vpc_v2 import Vpc, PrivateNetwork, SiteNetwork
...
class ComputeFileShare(ComputeProviderResource):
    ...
    def customize_list(controller, entities, *args, **kvargs):
        ...
        remote_servers = controller.get_directed_linked_resources_internal(
            resources=zone_insts_ids, link_type='relation', objdefs=objdefs, run_customize=True,
    ...
    def pre_create(controller, container, *args, **kvargs):
        ...
        steps = [
            ...
            {'step': ComputeFileShare.task_path + 'create_zone_share_step', 'args': [main_availability_zone, True]},
        ]

        for zone_id in availability_zones:
            step = {'step': ComputeFileShare.task_path + 'create_zone_share_step', 'args': [zone_id, False]}
            steps.append(step)
    ...
    def pre_import(controller, container, *args, **kvargs):
        ...
        steps = [
            ComputeFileShare.task_path + 'create_resource_pre_step',
            ComputeFileShare.task_path + 'create_compute_share_link_step',
            {'step': ComputeFileShare.task_path + 'import_zone_share_step', 'args': [main_availability_zone]},
    ...
    def pre_update(self, *args, **kvargs):
        ...
        # create task workflow
        steps = [
            ComputeFileShare.task_path + 'update_resource_pre_step'
        ]
        for zone_share in zone_shares:
            main = False
            if zone_share.parent_id == main_availability_zone:
                main = True
            subtask = {'step': ComputeFileShare.task_path + 'update_zone_share_step', 
...
class FileShare(AvailabilityZoneChildResource):
    ...
    def pre_create(controller, container, *args, **kvargs):
        ...
        orchestrator_idx = availability_zone.get_orchestrators_by_tag(orchestrator_tag,
                                                                      select_types=['vsphere', 'openstack', 'ontap'])
