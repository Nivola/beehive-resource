...
class LocalProviderResource(AsyncResource):
    ...
    def __get_linked_resources(self, link_type, container=None, objdef=None):
        ...
        try:
            ...
            res = self.manager.get_linked_resources_internal(
                resource=self.oid, link_type=link_type, container_id=container_id, objdef=objdef)
            resp = []
            for entity in res:
                entity_class = import_class(entity.objclass)
                obj = entity_class(self.controller, oid=entity.id, objid=entity.objid, name=entity.name,
    ...
    def get_physical_resource_from_container(self, cid, objdef):
        ...
        if len(res) == 0:
            raise ApiManagerError('No remote resource found for orchestrator %s and type %s' % (cid, objdef), code=404)
        res = res[0]
        return res
