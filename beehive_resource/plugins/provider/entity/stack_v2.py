...
class ComputeStackV2(ComputeProviderResource):
    ...
    def customize_list(controller, entities, *args, **kvargs):
        ...
        actions, tot = controller.get_resources(parent_list=parent_list, run_customize=False, authorize=False,
                                                type=ComputeStackAction.objdef, size=-1)
    ...
    def get_actions(self, name=None):
        ...
        if name is None:
            if self.actions is not None:
                ...
                actions, tot = self.controller.get_resources(parent=self.oid, run_customize=False, size=-1,
                                                             objdef=ComputeStackAction.objdef)
        else:
            actions, tot = self.controller.get_resources(parent=self.oid, name=name, run_customize=False, size=-1,
                                                         objdef=ComputeStackAction.objdef)
            actions = actions[0]
    ...
    def get_child_resources(self, uuid=None):
        ...
        if uuid is None:
            if self.resources is not None:
                ...
                resources, tot = self.get_linked_resources(link_type_filter='resource%', run_customize=False, size=-1,
                                                           authorize=False)
        else:
            resources, tot = self.get_linked_resources(link_type='resource.%s' % uuid, run_customize=False,
                                                       authorize=False)
    ...
    def pre_create(controller, container, *args, **kvargs):
        ...
        for action in actions:
            ...
            step = {'step': ComputeStackV2.task_path + 'run_stack_action_step', 'args': [action_name]}
            steps.append(step)
    ...
    def pre_delete(self, *args, **kvargs):
        ...
        for action in actions:
            if preserve is False:
                steps.append({'step': ComputeStackV2.task_path + 'remove_stack_resource_step', 'args': [action.oid]})
            steps.append({'step': ComputeStackV2.task_path + 'remove_stack_action_step', 'args': [action.oid]})
    ...
    @staticmethod
    def set_replica_args(data, name, replica, replica_arch_type, replica_role, replica_sync_type, replica_ip_master,
                         replica_user, replica_pwd, remove_replica):
        if replica:
