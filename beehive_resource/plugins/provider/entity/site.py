...
from beehive_resource.plugins.provider.entity.base import (
    LocalProviderResource,
    get_task,
)
...
class Site(LocalProviderResource):
    ...
    def add_orchestrator(self, *args, **kvargs):
        ...
        res = self.action(
            "add_orchestrator",
            steps,
            log="add orchestrator to site %s" % self.oid,
            check=None,
            **kvargs,
        )
        return res
    ...
    def delete_orchestrator(self, *args, **kvargs):
        ...
        for child in childs:
            res_physical = self.manager.get_linked_resources_internal(
                child.id, link_type="relation", container_id=c.oid
            )
        ...
        res = self.action(
            "del_orchestrator",
            steps,
            log="del orchestrator to site %s" % self.oid,
            check=None,
            **kvargs,
        )
        return res
