...
class ComputeGateway(ComputeProviderResource):
    ...
    def get_external_ip_address(self):
        ...
        for obj in objs:
            ...
            if edge is not None:
                edge_info = edge.info()
                uplink_vnics = [v for v in edge.get_vnics() if dict_get(v, 'isConnected') == 'true' and
                                dict_get(v, 'type') == 'uplink']
            ...
            if edge is not None:
                ...
                edge_info['attributes'].update({
                    'vnics': [{
                        ...
                        'mtu': dict_get(v, 'mtu'),
                    } for v in edge.get_vnics() if dict_get(v, 'isConnected') == 'true'],
                    'routes': routes
                })
    ...
    def detail(self):
        ...
        try:
            ...
            # get internal_router
            info['details'] = {
                'internal_router': self.get_internal_router_info()
            }
    ...
    def pre_create(controller, container, *args, **kvargs):
        ...
        # get vpcs
        uplink_vpc = controller.get_simple_resource(uplink_vpc_id)
        ...
        for availability_zone_id in availability_zones:
            ...
            if site_id == primary_site.oid:
                ...
                ip_address = None
                uplink_subnet = None
            step = {'step': ComputeGateway.task_path + 'create_zone_gateway_step',
                    'args': [availability_zone_id, role, uplink_network_id, transport_network_id, volume_flavor_id,
                             uplink_subnet, ip_address]}
            steps.append(step)
    ...
    def pre_delete(self, *args, **kvargs):
        ...
        # remove childs
        for child in childs:
            steps.append({'step': ComputeGateway.task_path + 'remove_child_step', 'args': [child]})
        # post expunge
    ...
    # nat
    #
    def __get_nat_rule_desc(self, action=None, original_address=None, translated_address=None, original_port=None,
    ...
    def add_nat_rule(self, action=None, original_address=None, translated_address=None, enabled=True, logged=False,
                     original_port=None, translated_port=None, protocol=None, vnic=None, role='default'):
        """Create nat rule
        ...
        if self.get_hypervisor() == 'vsphere':
            ...
            for gateway in gateways:
                ...
                # create nat rule
                edge = gateway.get_nsx_edge()
                desc = self.__get_nat_rule_desc(action, original_address, translated_address, original_port,
                                                translated_port, protocol, vnic)
                rule = edge.get_nat_rule(desc=desc)
                if rule is not None:
                    self.logger.warning('rule %s already exists' % desc)
                    continue

                edge.add_nat_rule(desc, action, original_address, translated_address, enabled=enabled, logged=logged,
    ...
    def del_nat_rule(self, action=None, original_address=None, translated_address=None, original_port=None,
                     translated_port=None, protocol=None, vnic=None, role='default'):
        """Delete nat rule
    ...
    #
    @staticmethod
    def get_firewall_rule_name(action, direction, source, dest, appl):
        name = '%s' % action
        ...
        if dest is not None:
            name = '%s-dst:%s' % (name, dest)
        if appl is not None:
            name = '%s-appl:%s' % (name, appl)
    ...
    def add_firewall_rule(self, action='accept', enabled=True, logged=False, direction=None, source=None, dest=None,
                          appl=None, role='default'):
        """Create firewall rule
        ...
        if self.get_hypervisor() == 'vsphere':
            ...
            for gateway in gateways:
                ...
                edge.add_firewall_rule(name, action=action, enabled=enabled, logged=logged, direction=direction,
    ...
    def del_firewall_rule(self, action=None, direction=None, source=None, dest=None, appl=None, role='default'):
