...
from beehive_resource.plugins.provider.entity.stack_v2 import (
    ComputeStackV2,
    ComputeStackAction,
    ComputeStackMysqlAction,
    ComputeStackPostgresqlAction,
)
...
class PostgresqlBase(object):
    engine_params = {
        ...
        "lvm": {
            "volume_group": {"data": "vg_data", "backup": "vg_backup"},
            "logical_volume": {"data": "lv_fsdata", "backup": "lv_backup"},
        },
        "mount_point": {
            "data": "/data",
            # 'backup': '/BCK_fisici'
...
class OracleBase(object):
    engine_params = {
        "versions": {
            "11EE": {"os_user": "ora11g", "oversion": "11g"},
            "12EE": {"os_user": "ora12c", "oversion": "12c"},
            "19EE": {"os_user": "ora19c", "oversion": "19c"},
        },
        "port": 1521,
        "hypervisor": "vsphere",
        "image": "Oracle12EE",
        ...
        "playbook": {"install": "CreateDboracle.yml", "delete": "DeleteDbOracle.yml"},
        ...
        "lsn_port": "1522",
        "service_users": {"zabbix": [{"name": "rou", "pwd": "Rou!142018", "host": "%", "privs": "*.*:SELECT"}]},
        "lvm": {
            "volume_group": {"data": "vg_oradata", "backup": "vg_orabck"},
            "logical_volume": {"data": "lv_oradata", "backup": "lv_orabck"},
        },
        "mount_point": {"data": "/oradata", "backup": "/BCK_fisici"},
    }
...
class SqlCreateHelper(SqlHelper):
    ...
    def check_params(self):
        ...
        self.compute_customization = self.container.get_simple_resource(
            customization, entity_class=ComputeCustomization
        )
        ...
        # get volume flavor
        self.volume_flavor = self.controller.get_resource(
            self.kvargs.pop("volume_flavor"),
        ...
        self.logger.debug("+++++ check_params - zabbix_host_groups: %s" % self.zabbix_host_groups)
        # per zabbix vm Windows
        self.zabbix_host_group = None
        if len(self.zabbix_host_groups) > 0:
            self.zabbix_host_group = self.zabbix_host_groups[0]
    ...
    def check_replica_params(self):
        ...
        if self.replica:
            ...
            resources, tot = master_stack.get_linked_resources(
    ...
    def run(self):
        ...
        self.stack_v2_params["enabled_monitoring"] = self.enabled_monitoring
    ...
    def action_create_server(self):
        res = {
            "name": "create_server1",
            "desc": "create server",
            "resource": {"type": "ComputeInstance", "operation": "create"},
            "params": {
                "name": "%s-server01" % self.name,
                "desc": "%s Server 01" % self.desc,
                "parent": self.compute_zone.oid,
                ...
                "networks": [
                    {
                        "subnet": self.subnet,
                        "vpc": self.vpc.oid,
                        "fixed_ip": {"hostname": self.hostname},
                    }
                ],
                "flavor": self.flavor.oid,
                "security_groups": [self.security_group.oid],
    ...
    def action_set_yum_proxy(self):
        res = {
            ...
            "params": {
                ...
                "extra_vars": {"set_proxy": self.set_proxy, "proxy_server": self.proxy},
            },
        }
        return res
    ...
    def action_enable_monitoring(self):
        self.enabled_monitoring = True
        self.logger.debug("+++++ AAA SqlCreateHelper - action_enable_monitoring")
        # vedi - set monitoring su ComputeInstance collegato
        res = {
            ...
            "params": {
                ...
                "extra_vars": {
                    ...
                    "p_custom_host_group": self.zabbix_host_group,
                    "p_custom_templates": [],
...
class OracleCreateHelperNew(SqlCreateHelper):
    ...
    def internal_run(self):
        ...
        if self.oracle_bck_disk_size is not None:
        ...
        self.actions.append(
            self.action_setup_volume(
                boot_index=2,
                data_dir=self.ora_dataf_path,
                data_device=ora_data_device,
                volume_group=self.lvm_vg_data,
                logical_volume=self.lvm_lv_data,
            )
        )
        ...
        self.actions.append(
            self.action_setup_volume(
                boot_index=3,
                data_dir=self.backup_dir,
                data_device=bck_data_device,
                volume_group=self.lvm_vg_backup,
                logical_volume=self.lvm_lv_backup,
            )
        )
...
class SqlImportHelper(SqlHelper):
    ...
    def action_import_server(self):
        return {
            "name": "import_server1",
            "desc": "import server",
            "resource": {"type": "ComputeInstance", "operation": "import"},
            "params": {
...
class SqlUpdateHelper(SqlHelper):
    ...
    def check_params(self):
        ...
        if customization is None:
            customization = dict_get(self.engine_params, "customization")
        self.compute_customization = self.stack.container.get_simple_resource(
            customization, entity_class=ComputeCustomization
        )
    ...
    def check_replica_params(self):
        ...
        resources = self.stack.controller.get_directed_linked_resources_internal(
            resources=[self.stack.oid],
            link_type="resource%",
            objdef=ComputeInstance.objdef,
        )
    ...
    def run(self):
        ...
        if self.remove_replica:
            ...
            self.stack.set_configs(
                key="users.replica.password",
    ...
    def action_config_replica_mysql(self):
        res = {
            ...
            "params": {
                ...
                "parent": self.compute_customization.oid,
                "compute_zone": self.compute_zone.oid,
                "instances": [{"id": self.compute_instance.oid, "extra_vars": {}}],
                "playbook": self.playbook,
                "extra_vars": {
                    "p_mysql_repo_version": self.engine_major_version,
                    "p_mysql_root_username": self.admin_user,
...
class SqlActionHelper(SqlHelper):
    ...
    def enable_monitoring(self, **kwargs):
        ...
        resources = self.compute_instance.controller.get_directed_linked_resources_internal(
            resources=[self.compute_instance.oid],
            link_type="image",
            objdef=ComputeImage.objdef,
        )
    ...
    def haproxy_deregister(self, **kwargs):
        ...
        job_template_args["name"] = "%s-deregister_from_haproxy-jobtemplate-%s" % (
            self.stack.name,
            id_gen(10),
        )
    ...
    def run_server_action(self, **kwargs):
        ...
        # self.logger.debug("+++++ run_server_action - extra_vars: %s" % extra_vars)
...
class PostgresqlActionHelper(SqlActionHelper):
    ...
    def start(self, **kvargs):
        ...
        # - wait server is up and running
        data = {"customization": "os-utility", "playbook": "wait_ssh_is_up.yml"}
        steps.append(
            {
                "step": ComputeStackAction.task_path + "sql_invoke_apply_customization_step",
                "args": [self.compute_instance.oid, data],
    ...
    def __run_pgsql_query(self, query, database="postgres"):
        ...
        cmd = "su - postgres -c \"psql -c \\\"%s\\\" -At -R '+++' -F '|' -d '%s'\"" % (
            query,
            database,
        )
...
class SqlComputeStackV2(ComputeStackV2):
    ...
    def get_compute_instance(self):
        ...
        # get compute instance linked to stack
        resources = self.controller.get_directed_linked_resources_internal(
            resources=[self.oid], link_type="resource%", objdef=ComputeInstance.objdef
        )
                    ...
                    (
                        {
                            "type": None,
                            "name": k,
                            "password": self.controller.decrypt_data(v),
                        }
                    )
                )
        return res
    ...
    def pre_create(controller, container, *args, **kvargs):
        ...
        :raise ApiManagerError:
        """
        helper_class = SqlCreateHelper(controller, container, *args, **kvargs).get_helper_by_engine(
            kvargs.get("engine")
        )
