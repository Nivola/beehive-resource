...
class CreateVpcParamRequestSchema(CreateProviderResourceRequestSchema):
    ...
    cidr = fields.String(required=True, example='192.168.200.0/21', description='vpc cidr. Ex. 192.168.200.0/21')
    type = fields.String(required=False, example='private', default='private', missing='private',
                         description='vpc type. Can be shared or private', validate=OneOf(['shared', 'private']))
...
class DelSiteNetworkSubnetsResponseSchema(Schema):
    ...
    orchestrator_tag = fields.String(required=False, missing='default', description='orchestrator tag. Use to select a '
                                     'subset of orchestrators where entity must be created.')
...
class ListVpcNetworks(ProviderVpc):
    ...
    responses = SwaggerApiView.setResponses({
        200: {
            'description': 'success',
            'schema': ListVpcNetworksResponseSchema
        }
    })
...
class AddVpcPrivateNetworkRequestSchema(Schema):
    cidr = fields.String(required=False, example='192.168.200.0/23', missing='192.168.200.0/23',
