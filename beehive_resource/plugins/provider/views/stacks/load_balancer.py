...
class CreateLoadBalancerParamRequestSchema(CreateProviderResourceRequestSchema):
    ...
    engine = fields.String(
        ...
        validate=OneOf(["haproxy"]),
    )
    engine_configs = fields.Nested(
        CreateLoadBalancerParamConfigRequestSchema,
        required=True,
...
class CreateLoadBalancer(ProviderLoadBalancer):
    ...
    def post(self, controller, data, *args, **kwargs):
        ...
        for item in link_params:
            resource.add_link(
                name="%s-%s-%s-%s-link" % (resource.oid, item[1].oid, item[0], id_gen()),
                type=item[0],
                end_resource=item[1].oid,
                attributes={},
            )
...
class LoadBalancerProviderAPI(ProviderAPI):
    ...
    def register_api(module, **kwargs):
        ...
        rules = [
            ...
            (
                "%s/load_balancers/<oid>/resources" % base,
                "GET",
                GetLoadBalancerResources,
                {},
            ),
