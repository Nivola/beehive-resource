...
class ListRulesRequestSchema(ListResourcesRequestSchema):
    ...
    source = fields.String(required=False, allow_none=True, description='a dictionary with source type and value. '
                                                                        'Syntax SecurityGroup:<uuid> or Cidr:<uuid>')
    destination = fields.String(required=False, allow_none=True,
                                description='a dictionary with destination type and value. '
                                            'Syntax SecurityGroup:<uuid> or Cidr:<uuid>')
    service = fields.String(required=False, allow_none=True, description='describe protocol and ports to use in rule.'
                                                                         'Syntax <proto>:<port>')
...
class CreateRuleSourceRequestSchema(Schema):
    type = fields.String(required=True, example='SecurityGroup',
...
class CreateRuleServiceRequestSchema(Schema):
    port = fields.String(required=False, example='*', description='can be an integer between 0 and 65535 or a range '
                         'with start and end in the same interval. Range format is <start>-<end>')
    protocol = fields.String(required=False, example='*',
...
class CreateRuleParamRequestSchema(CreateProviderResourceRequestSchema):
    ...
    source = fields.Nested(CreateRuleSourceRequestSchema, required=True, allow_none=True,
                           description='a dictionary with source type and value.')
    destination = fields.Nested(CreateRuleSourceRequestSchema, required=True, allow_none=True,
                                description='a dictionary with destination type and value.')
    service = fields.Nested(CreateRuleServiceRequestSchema, required=True, allow_none=True,
                            description='describe protocol and ports to use in rule')
    reserved = fields.Boolean(required=False, missing=False,  description='Flag to use when rule must be reserved '
                                                                          'to admin management')
