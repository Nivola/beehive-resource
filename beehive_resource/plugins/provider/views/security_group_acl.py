...
class ListSecurityGroupAclsRequestSchema(ListResourcesRequestSchema):
    ports = fields.String(required=False, example='80', missing=None, description='Comma separated list of ports')
    protocol = fields.String(required=False, example='tcp', missing=None,
    ...
    source = fields.String(required=False, example='10.102.0.0/24', missing=None,
...
class ListSecurityGroupAclsResponseSchema(PaginatedResponseSchema):
    security_group_acls = fields.Nested(ListSecurityGroupAclsParamsResponseSchema, many=True, required=True,
                                        allow_none=True)
...
class ListSecurityGroupAcls(ProviderSecurityGroupAcl):
    ...
    responses = SwaggerApiView.setResponses({
        200: {
            'description': 'success',
            'schema': ListSecurityGroupAclsResponseSchema
        }
    })
...
class GetSecurityGroupAcl(ProviderSecurityGroupAcl):
    ...
    responses = SwaggerApiView.setResponses({
        200: {
            'description': 'success',
            'schema': GetSecurityGroupAclResponseSchema
        }
    })
...
class CreateSecurityGroupAclSourceRequestSchema(Schema):
    type = fields.String(required=True, example='SecurityGroup', validate=OneOf(['SecurityGroup', 'Cidr']),
                         description='Source/destination type supported: SecurityGroup, Instance, Cidr')
    value = fields.String(required=True, example='3151',
...
class CreateSecurityGroupAclServiceRequestSchema(Schema):
    ports = fields.String(required=False, example='*', missing='*',
                          description='Comma separated list of ports. Ex. 80, 80,8080. Use * for all the ports')
    protocol = fields.String(required=False, example='*', missing='*',
                             description='Supported **protocol** are only 6 [tcp], 17 [udp], 1 [icmp], * [all]')
...
class CreateSecurityGroupAclParamRequestSchema(CreateProviderResourceRequestSchema):
    ...
    source = fields.Nested(CreateSecurityGroupAclSourceRequestSchema, required=False, allow_none=True,
                           description='Acl source')
    service = fields.Nested(CreateSecurityGroupAclServiceRequestSchema, required=False, allow_none=True,
...
class SecurityGroupAclProviderAPI(ProviderAPI):
    ...
    def register_api(module, **kwargs):
        ...
        rules = [
            ...
            ('%s/security_group_acls/<oid>' % base, 'DELETE', DeleteSecurityGroupAcl, {}),
        ]
