...
class ListSecurityGroupsResponseSchema(PaginatedResponseSchema):
    security_groups = fields.Nested(
        ListSecurityGroupsParamsResponseSchema,
        many=True,
        required=True,
        allow_none=True,
    )
...
class HasSecurityGroupAclRequestSchema(Schema):
    ...
    ports = fields.String(
        ...
        example="*",
        missing="*",
        description="Comma separated acl ports (80,8000), single port (80) or port interval " "(4001-4002). * for all",
    )
...
class HasSecurityGroupAcl(ProviderSecurityGroup):
    ...
    responses = SwaggerApiView.setResponses(
        {200: {"description": "success", "schema": HasSecurityGroupAclResponseSchema}}
    )
...
class SecurityGroupProviderAPI(ProviderAPI):
    ...
    def register_api(module, **kwargs):
        ...
        rules = [
            ...
            (
                "%s/security_groups/<oid>/zabbix" % base,
                "POST",
                CreateSecurityGroupZabbixRule,
                {},
            ),
            (
                "%s/security_groups/<oid>/zabbix" % base,
                "DELETE",
                DeleteSecurityGroupZabbixRule,
                {},
            ),
            ("%s/security_groups/<oid>/acls" % base, "GET", ListSecurityGroupAcls, {}),
            (
                "%s/security_groups/<oid>/acls/check" % base,
                "GET",
                HasSecurityGroupAcl,
                {},
            ),
