...
from beehive.common.apimanager import PaginatedResponseSchema, SwaggerApiView, \
    GetApiObjectRequestSchema, CrudApiObjectTaskResponseSchema, CrudApiTaskResponseSchema, ApiManagerError
...
class BastionAttributesResponseSchema(Schema):
    ...
    type = fields.String(required=True, example='openstack', description='bastion type: vsphere or openstack')


class BastionResponseSchema(ResourceResponseSchema):
    flavor = fields.Nested(BastionFlavorResponseSchema, required=True, description='flavor', allow_none=True)
    image = fields.Nested(BastionImageResponseSchema, required=True, description='image', allow_none=True)
    vpcs = fields.Nested(BastionNetworkResponseSchema, required=True, many=True, description='vpcc list',
                         allow_none=True)
    block_device_mapping = fields.Nested(BastionBlockDeviceResponseSchema, required=True, many=True,
                                         description='block device mapping list', allow_none=True)
    attributes = fields.Nested(BastionAttributesResponseSchema, required=True, description='custom attributes',
                               allow_none=True)
...
class ListBastions(ProviderBastion):
    ...
    responses = SwaggerApiView.setResponses({
        200: {
            'description': 'success',
            'schema': ListBastionsResponseSchema
        }
    })
...
class GetBastion(ProviderBastion):
    ...
    responses = SwaggerApiView.setResponses({
        200: {
            'description': 'success',
            'schema': GetBastionResponseSchema
        }
    })
...
class CreateBastionNetworkIpRequestSchema(Schema):
    ...
    hostname = fields.String(required=False, example='bastion-vsphere01.tstsddc.csi.it', description='host name')
...
class CreateBastionParamRequestSchema(CreateProviderResourceRequestSchema):
    ...
    host_group = fields.String(required=False, example='default', missing='default',
                               description='Define the optional host group where put the bastion')
    flavor = fields.String(required=False, example='vm.s1.micro', missing='vm.s1.micro',
                           description='id or uuid of the flavor')
    volume_flavor = fields.String(required=False, example='vol.default', missing='vol.default',
                                  description='id or uuid of the volume flavor')
    image = fields.String(required=False, example='Centos7', missing='Centos7', description='id or uuid of the image')
    admin_pass = fields.String(required=False, example='test', missing=None, description='admin password to set')
    key_name = fields.String(required=False, example='bastion-key', missing='bastion-key',
                             description='ssh key name or uuid')
    acl = fields.Nested(CreateBastionAclRequestSchema, many=True, required=False, allow_none=True,
...
class SendBastionActionParamsMigrateRequestSchema(Schema):
    live = fields.Boolean(required=False, missing=False, default=False,
...
class SendBastionActionParamsEnableMonitoringRequestSchema(Schema):
    host_group = fields.String(required=False, missing='PrivateBastionHost', allow_none=True,
...
class SendBastionActionParamsRequestSchema(Schema):
    ...
    enable_monitoring = fields.Nested(SendBastionActionParamsEnableMonitoringRequestSchema,
    ...
    set_flavor = fields.Nested(SendBastionActionParamsSetFlavorRequestSchema, description='set flavor to server')
    ...
    add_security_group = fields.Nested(SendBastionActionParamsSgRequestSchema,
                                       description='add security group to server')
    del_security_group = fields.Nested(SendBastionActionParamsSgRequestSchema,
