...
from beehive.common.apimanager import ApiManagerError, CrudApiTaskResponseSchema, PaginatedResponseSchema, SwaggerApiView, GetApiObjectRequestSchema, \
...
class ListMonitoringFolders(ProviderMonitoringFolder):
    ...
    responses = SwaggerApiView.setResponses({
        200: {
            'description': 'success',
            'schema': ListMonitoringFoldersResponseSchema
        }
    })
...
class GetMonitoringFolder(ProviderMonitoringFolder):
    ...
    responses = SwaggerApiView.setResponses({
        200: {
            'description': 'success',
            'schema': GetMonitoringFolderResponseSchema
        }
    })
...
class CreateMonitoringFolderGrafanaFolderRequestSchema(Schema):
    ...
    desc = fields.String(required=True, example='This is the test folder', default='', description='Folder description')
...
class CreateMonitoringFolderParamRequestSchema(CreateProviderResourceRequestSchema):
    ...
    grafana_folder = fields.Nested(CreateMonitoringFolderGrafanaFolderRequestSchema, required=True,
...
class SendMonitoringFolderActionParamsDashboardRequestSchema(Schema):
    ...
    dashboard_to_search = fields.String(required=True, example='dashboard-to-search', description='dashboard name to add')
    dash_tag = fields.String(required=False, allow_none=True, example='abc,def', description='tags dashboard')
    organization = fields.String(required=False, example='organization', description='organization to replace in dashboard')
    division = fields.String(required=False, example='division', description='division to replace in dashboard')
...
class SendMonitoringFolderActionParamsPermissionRequestSchema(Schema):
    # folder_id_from = fields.String(required=False, allow_none=True, example='default', description='folder from copy dashboard')
    team_viewer = fields.String(required=True, example='team_01', description='team viewer name to add permission')
    team_editor = fields.String(required=False, allow_none=True, example='team_02', description='team editor name to add permission')


class SendMonitoringFolderActionParamsRequestSchema(Schema):
    add_dashboard = fields.Nested(SendMonitoringFolderActionParamsDashboardRequestSchema, required=False, allow_none=True,
        description='add dashboard to folder')
    add_permission = fields.Nested(SendMonitoringFolderActionParamsPermissionRequestSchema, required=False, allow_none=True,
...
class SendMonitoringFolderActionRequestSchema(Schema):
    ...
    schedule = fields.Dict(required=False, missing=None, description='schedule to use when you want to run a scheduled action')
...
class ComputeMonitoringFolderAPI(ProviderAPI):
    ...
    def register_api(module, **kwargs):
        ...
        rules = [
            ...
            ('%s/monitoring_folders/<oid>' % base, 'DELETE', DeleteMonitoringFolder, {}),

            ('%s/monitoring_folders/<oid>/actions' % base, 'PUT', SendMonitoringFolderAction, {}),
        ]
        kwargs['version'] = 'v1.0'
