...
class ProviderVsphere(AbstractProviderHelper):
    def type_mapping(self, mtype, mvalue):
        ...
        try:
            if mtype == 'Network':
                ...
                res = {'type': 'DistributedVirtualPortgroup', 'value': net.ext_id, 'name': None}
            elif mtype == 'Cidr':
    ...
    def create_folder(self, datacenter, folder):
        ...
        prepared_task = self.create_resource(VsphereFolder, name=name, desc=self.resource.desc, active=False,
                                             attribute={}, parent=None, tags='', folder_type='vm',
                                             datacenter=datacenter, folder=folder)
    ...
    def create_security_group(self, parent):
        ...
        prepared_task = self.create_resource(NsxSecurityGroup, name=name, desc=self.resource.desc, active=False,
                                             attribute={}, parent=None, tags='')
    ...
    def create_section(self):
        ...
        resource_model = self.container.add_resource(objid=objid, name=name, resource_class=CustomResource,
                                                     ext_id=section_id, active=True, desc=self.resource.desc,
    ...
    def create_nsx_rule(self, section_id, name, direction, source, dest, service, appliedto, logged=True):
        ...
        resource_model = self.container.add_resource(objid=objid, name=name, resource_class=CustomResource,
                                                     ext_id=rule_id, active=True, desc=desc, attrib=attribs,
                                                     parent=None, tags=['vsphere'])
    ...
    def create_server(self, folder, params):
        ...
        try:
            ...
            server_conf['customization_spec_name'] = image_attribs.get('customization_spec_name',
                                                                       'WS201x PRVCLOUD custom OS sysprep')
                ...
                if from_template is False:
                    ...
                    for template_disk in template_disks:
                        if template_disk['size'] == remote_volume.get_size():
                            ...
                            server.add_link('%s-%s-volume-link' % (server.oid, remote_volume.oid), 'volume',
                                            remote_volume.oid, attributes={'boot': False})
    ...
    def ipset_action(self, action, params):
        ...
        try:
            ...
            prepared_task, code = action_func(**params)
            res = self.run_sync_task(prepared_task, msg='Run action %s over ipset %s' % (action, remote_ipset.uuid))
            return res
        except Exception as ex:
            self.logger.error(ex, exc_info=False)
            raise TaskError(ex)
    ...
    def create_ipset(self, fixed_ip, rule_groups):
        ...
        try:
            ...
            for remote_sg in remote_sgs:
                ...
                self.run_sync_task(prepared_task, msg='assign ipset %s to security group %s' %
                                                      (ipset.uuid, remote_sg.uuid))
    ...
    def create_image(self, image_id, template_pwd, guest_id, customization_spec_name):
        ...
        :raise ApiManagerError: :class:`ApiManagerError`
        """
        attrib = {'reuse': True, 'template_pwd': template_pwd, 'guest_id': guest_id,
    ...
    def create_gateway(self, role, uplink_ip_address, transport_ip_address, params):
        ...
        if uplink_network is not None and uplink_subnet is not None:
            # get ippool id from subnet config
            subnet_pool1 = uplink_subnet.get('allocation_pools_vs', None)
            subnet_pool2 = uplink_subnet.get('vsphere_id', None)
            subnet_pool = subnet_pool1 or subnet_pool2
        ...
        gateway = self.get_resource(gateway_id)
            ...
            for child in childs:
                ...
                try:
                    if definition in [VsphereFolder.objdef, NsxSecurityGroup.objdef, VsphereDvpg.objdef,
                                      VsphereServer.objdef, NsxIpSet.objdef, VsphereVolume.objdef,
                                      VsphereVolumeType.objdef, NsxLogicalSwitch.objdef, NsxEdge.objdef]:
                        ...
                        if sub_type == NsxDfwRule.objdef:
                            ...
                            if rule == {}:
                                ...
                                self.run_sync_task(prepared_task, msg='remove dfw rule %s' % attribs['id'])
                        ...
                        # delete dfw section
                            ...
                            self.run_sync_task(prepared_task, msg='remove dfw section %s' % attribs['id'])
