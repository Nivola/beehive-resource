...
from beehive_resource.plugins.provider.entity.monitoring_folder import ComputeMonitoringFolder, MonitoringFolder
...
class ComputeMonitoringFolderTask(AbstractProviderResourceTask):
    ...
    def create_zone_monitoring_folder_step(task, step_id, params, availability_zone_id, *args, **kvargs):
        ...
        logger.debug('create_zone_monitoring_folder_step - monitoring_folder_params {} '.format(monitoring_folder_params))
        ...
        compute_monitoring_folder.add_link('%s-monitoring_folder-link' % monitoring_folder_id, 'relation.%s' % site_id,
                                       monitoring_folder_id, attributes={})
        task.progress(step_id, msg='Link monitoring_folder %s to compute_monitoring_folder %s' % (monitoring_folder_id, oid))

        # wait for task to complete
        run_sync_task(prepared_task, task, step_id)
        task.progress(step_id, msg='Create monitoring folder %s in availability_zone %s'
                                   % (monitoring_folder_id, availability_zone_id))
...
class MonitoringFolderTask(AbstractProviderResourceTask):
    ...
    def create_grafana_folder_step(task, step_id, params, *args, **kvargs):
        ...
        try:
            ...
            monitoring_folder.add_link('%s-grafana_folder-link' % folder_id, 'relation', folder_id, attributes={'reuse': True})
            ...
            if norescreate is not None and norescreate == True:
                logger.error('create_grafana_folder_step - folder NON trovato - name: %s' % grafana_folder_name)
                logger.error('create_grafana_folder_step - link non creato - MonitoringFolder - oid: %s' % oid)
                raise Exception('folder NON trovato - name: %s' % grafana_folder_name)
            else:
                logger.debug('create_grafana_folder_step - folder NON trovato - name: %s' % grafana_folder_name)
                ...
                monitoring_folder: MonitoringFolder
                monitoring_folder = task.get_simple_resource(oid)
                monitoring_folder.add_link('%s-grafana_folder-link' % folder_id, 'relation', folder_id, attributes={})
                task.progress(step_id, msg='Link grafana_folder %s to monitoring_folder %s' % (folder_id, oid))
    ...
    def monitoring_folder_action_step(task, step_id, params, *args, **kvargs):
        ...
        if action == 'add_dashboard':
            ...
            # folder_uid_to = params.get('folder_uid_to')
            ...
            res, str = grafana_folder.add_dashboard(dashboard_to_search, folder_uid_to, organization, division, account, dash_tag, params)
