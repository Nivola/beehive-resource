...
class AppliedComputeCustomizationTask(AbstractProviderResourceTask):
    ...
    def create_zone_customization_step(task, step_id, params, availability_zone_id, project, *args, **kvargs):
        ...
        for instance in instances:
            ...
            obj = abstractResourceTask.get_resource(instance['id'])
            zone_objs, total = obj.get_linked_resources(link_type_filter='relation.%s' % obj.availability_zone_id)
            ...
            if obj_availability_zone_id == availability_zone_id:
                ...
                inst_extra_vars.update({
                    ...
                    'ansible_port': obj.get_real_ssh_port(),
                    # 'ansible_host': obj.get_ip_address(),
                    # 'ansible_pipelining': True,
                    'ansible_connection': 'ssh',
                    'ansible_ssh_common_args': '-o UserKnownHostsFile=/dev/null -o StrictHostKeyChecking=no'
                })
                inst_extra_vars = obj.set_ansible_ssh_common_args(inst_extra_vars)

                # get instance ip address
                ip_address = obj.get_real_ip_address()
        ...
        if len(hosts) == 0:
            task.progress(step_id, msg='no hosts found in availability zone %s' % availability_zone_id)
            return False, params
        ...
        awx_job_template = {
            ...
            'playbook': playbook,
            'verbosity': verbosity,
            'ssh_creds': obj.get_real_admin_credential(),
        ...
        applied_compute_customization.add_link('%s-applied-customization-link' % customization_id,
                                               'relation.%s' % site_id, customization_id, attributes={})
        task.progress(step_id, msg='Link applied_customization %s to applied_compute_customization %s'
                                   % (customization_id, oid))

        # wait for task to complete
        run_sync_task(prepared_task, task, step_id)
        task.progress(step_id, msg='Create applied_customization %s in availability_zone %s'
                                   % (customization_id, availability_zone_id))
...
class AppliedCustomizationTask(AbstractProviderResourceTask):
    ...
    def create_awx_job_template_step(task, step_id, params, *args, **kvargs):
        ...
        applied_customization.add_link('%s-awx_job_template-link' % job_template_id, 'relation', job_template_id,
                                       attributes={})
        task.progress(step_id, msg='Link awx_job_template %s to applied_customization %s' % (job_template_id, oid))
