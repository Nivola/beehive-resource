...
class ComputeLoggingRoleTask(AbstractProviderResourceTask):
    ...
    def create_zone_logging_role_step(task, step_id, params, availability_zone_id, *args, **kvargs):
        ...
        compute_logging_role.add_link('%s-logging_role-link' % logging_role_id, 'relation.%s' % site_id,
                                       logging_role_id, attributes={})
        task.progress(step_id, msg='Link logging_role %s to compute_logging_role %s' % (logging_role_id, oid))

        # wait for task to complete
        run_sync_task(prepared_task, task, step_id)
        task.progress(step_id, msg='Create logging role %s in availability_zone %s'
                                   % (logging_role_id, availability_zone_id))
...
class LoggingRoleTask(AbstractProviderResourceTask):
    ...
    def create_elk_role_step(task, step_id, params, *args, **kvargs):
        ...
        try:
            ...
            logging_role.add_link('%s-elk_role-link' % role_id, 'relation', role_id, attributes={ 'reuse': True })
