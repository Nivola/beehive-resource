...
from beehive_resource.plugins.provider.entity.monitoring_team import ComputeMonitoringTeam, MonitoringTeam
...
class ComputeMonitoringTeamTask(AbstractProviderResourceTask):
    ...
    def create_zone_monitoring_team_step(task, step_id, params, availability_zone_id, *args, **kvargs):
        ...
        compute_monitoring_team.add_link('%s-monitoring_team-link' % monitoring_team_id, 'relation.%s' % site_id,
                                       monitoring_team_id, attributes={})
        task.progress(step_id, msg='Link monitoring_team %s to compute_monitoring_team %s' % (monitoring_team_id, oid))

        # wait for task to complete
        run_sync_task(prepared_task, task, step_id)
        task.progress(step_id, msg='Create monitoring team %s in availability_zone %s'
                                   % (monitoring_team_id, availability_zone_id))
...
class MonitoringTeamTask(AbstractProviderResourceTask):
    ...
    def create_grafana_team_step(task, step_id, params, *args, **kvargs):
        ...
        try:
            ...
            monitoring_team.add_link('%s-grafana_team-link' % team_id, 'relation', team_id, attributes={ 'reuse': True })
            ...
            if norescreate is not None and norescreate == True:
                logger.error('create_grafana_team_step - team NON trovato - name: %s' % grafana_team_name)
                logger.error('create_grafana_team_step - link non creato - MonitoringTeam - oid: %s' % oid)
                raise Exception('team NON trovato - name: %s' % grafana_team_name)
            else:
                logger.debug('create_grafana_team_step - team NON trovato - name: %s' % grafana_team_name)
                ...
                task.progress(step_id, msg='Link grafana_team %s to monitoring_team %s' % (team_id, oid))
