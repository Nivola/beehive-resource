...
from beehive_resource.plugins.provider.entity.monitoring_alert import ComputeMonitoringAlert, MonitoringAlert
...
class ComputeMonitoringAlertTask(AbstractProviderResourceTask):
    ...
    def create_zone_monitoring_alert_step(task, step_id, params, availability_zone_id, *args, **kvargs):
        ...
        compute_monitoring_alert.add_link('%s-monitoring_alert-link' % monitoring_alert_id, 'relation.%s' % site_id,
                                       monitoring_alert_id, attributes={})
        task.progress(step_id, msg='Link monitoring_alert %s to compute_monitoring_alert %s' % (monitoring_alert_id, oid))

        # wait for task to complete
        run_sync_task(prepared_task, task, step_id)
        task.progress(step_id, msg='Create monitoring alert %s in availability_zone %s'
                                   % (monitoring_alert_id, availability_zone_id))
...
class MonitoringAlertTask(AbstractProviderResourceTask):
    ...
    def create_grafana_alert_step(task, step_id, params, *args, **kvargs):
        ...
        try:
            ...
            monitoring_alert.add_link('%s-grafana_alert-link' % alert_id, 'relation', alert_id, attributes={ 'reuse': True })
            ...
            if norescreate is not None and norescreate == True:
                logger.error('create_grafana_alert_step - alert NON trovato - name: %s' % grafana_alert_name)
                logger.error('create_grafana_alert_step - link non creato - MonitoringAlert - oid: %s' % oid)
                raise Exception('alert NON trovato - name: %s' % grafana_alert_name)
            else:
                logger.debug('create_grafana_alert_step - alert NON trovato - name: %s' % grafana_alert_name)
                ...
                prepared_task, code = grafana_container.resource_factory(GrafanaAlertNotification, **grafana_alert_params)
                ...
                monitoring_alert = task.get_simple_resource(oid)
                monitoring_alert.add_link('%s-grafana_alert-link' % alert_id, 'relation', alert_id, attributes={})
                task.progress(step_id, msg='Link grafana_alert %s to monitoring_alert %s' % (alert_id, oid))
