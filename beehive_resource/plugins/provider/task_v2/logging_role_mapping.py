...
class ComputeLoggingRoleMappingTask(AbstractProviderResourceTask):
    ...
    def create_zone_logging_role_mapping_step(task, step_id, params, availability_zone_id, *args, **kvargs):
        ...
        logger.debug(
            "+++++ create_zone_logging_role_mapping_step - logging_role_mapping_params {} ".format(
                logging_role_mapping_params
            )
        )
        ...
        compute_logging_role_mapping.add_link(
            "%s-logging_role_mapping-link" % logging_role_mapping_id,
            "relation.%s" % site_id,
            logging_role_mapping_id,
            attributes={},
        )
        task.progress(
            step_id,
            msg="Link logging_role_mapping %s to compute_logging_role_mapping %s" % (logging_role_mapping_id, oid),
        )
        ...
        task.progress(
            step_id,
            msg="Create logging role mapping %s in availability_zone %s"
...
class LoggingRoleMappingTask(AbstractProviderResourceTask):
    ...
    def create_elk_role_mapping_step(task, step_id, params, *args, **kvargs):
        ...
        try:
            ...
            logger.debug(
                "+++++ create_elk_role_mapping_step - role_mapping trovato - aggiungo link: %s" % role_mapping_id
            )
            ...
            logging_role_mapping.add_link(
                "%s-elk_role_mapping-link" % role_mapping_id,
                "relation",
                role_mapping_id,
            ...
            if norescreate is not None and norescreate == True:
                logger.error(
                    "create_elk_role_mapping_step - role_mapping NON trovato - name: %s" % elk_role_mapping_name
                )
                ...
                task.progress(
                    step_id,
                    msg="Link elk_role_mapping %s to logging_role_mapping %s" % (role_mapping_id, oid),
                )
