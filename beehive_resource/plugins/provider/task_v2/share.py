...
from beehive_resource.plugins.provider.task_v2 import AbstractProviderResourceTask, orchestrator_mapping
...
class ComputeFileShareTask(AbstractProviderResourceTask):
    ...
    @staticmethod
    @task_step()
    def create_compute_share_link_step(task, step_id, params, *args, **kvargs):
    ...
    @staticmethod
    @task_step()
    def create_zone_share_step(task, step_id, params, availability_zone_id, main, *args, **kvargs):
        """Create zone share
        ...
        task.progress(step_id, msg='Create share %s in availability zone %s' % (share_id, availability_zone_id))
    
        return share_id, params
    
    @staticmethod
    @task_step()
    def import_zone_share_step(task, step_id, params, availability_zone_id, *args, **kvargs):
        """Import zone share
        ...
        share_id = prepared_task['uuid']
        run_sync_task(prepared_task, task, step_id)
        task.progress(step_id, msg='Import share %s in availability zone %s' % (share_id, availability_zone_id))
    
        return share_id, params
    
    @staticmethod
    @task_step()
    def update_zone_share_step(task, step_id, params, zone_share_id, *args, **kvargs):
        ...
        if size is not None:
            ...
            run_sync_task(prepared_task, task, step_id)
            task.progress(step_id, msg='Update share %s size in availability zone %s' % (oid, avz_id))
        ...
        if grant is not None:
            ...
            run_sync_task(prepared_task, task, step_id)
            task.progress(step_id, msg='Update share %s grant in availability zone %s' % (oid, avz_id))
    
        return zone_share_id, params
    
    @staticmethod
    @task_step()
    def link_share_step(task, step_id, params, *args, **kvargs):
    ...
    @staticmethod
    @task_step()
    def create_main_share_step(task, step_id, params, *args, **kvargs):
        """Create zone share
    ...
    @staticmethod
    @task_step()
    def import_main_share_step(task, step_id, params, *args, **kvargs):
        """Import main share
