...
class ComputeBastionTask(AbstractProviderResourceTask):
    ...
    def create_gateway_nat_step(task, step_id, params, *args, **kvargs):
        ...
        # add nat rule
        gw.add_nat_rule(action='dnat', original_address=nat_ip_address, translated_address=resource.get_ip_address(),
                        original_port=nat_port, translated_port=22, protocol='tcp', vnic=0, role='default',
                        enabled=True, logged=False)
        ...
        gw.add_firewall_rule(action='accept', enabled=True, logged=False, direction=None, source=source,
                             dest=None, appl=appl, role='default')
    ...
    def create_user_gateway_step(task, step_id, params, *args, **kvargs):
        ...
        prepared_task, code = resource.action('add_user', user_name='gateway', user_pwd=random_password(length=20),
                                              user_ssh_key=key_name, sync=True)
        run_sync_task(prepared_task, task, step_id)
    ...
    def delete_gateway_nat_step(task, step_id, params, *args, **kvargs):
        ...
        # add nat rule
        gw.del_nat_rule(action='dnat', original_address=nat_ip_address, translated_address=resource.get_ip_address(),
                        original_port=nat_port, translated_port=22, protocol='tcp', vnic=0, role='default')
        ...
        for acl in acls:
            ...
            gw.del_firewall_rule(action='accept', enabled=True, logged=False, direction=None, source=source,
                                 dest=None, appl=None, role='default')
