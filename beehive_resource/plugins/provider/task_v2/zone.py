...
class ZoneTask(AbstractProviderResourceTask):
    ...
    def compute_zone_set_quotas_step(task, step_id, params, *args, **kvargs):
        ...
        for zone in zones:
            zone_resource = task.get_resource(zone)
            if zone_resource.is_active() is False:
                task.progress(step_id, msg='compute zone %s availability zone %s is not active' % (oid, zone))
            else:
                ...
                task.progress(step_id, msg='Update compute zone %s availability zone %s quotas: %s' %
                                           (oid, zone, quotas))
