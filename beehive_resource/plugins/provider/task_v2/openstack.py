...
class ProviderOpenstack(AbstractProviderHelper):
    QUOTAS = {
        ...
        'compute.security_groups': {'type': 'network', 'quota': 'security_group', 'factor': 1},
        'compute.security_group_rules': {'type': 'network', 'quota': 'security_group_rule', 'factor': 1},
    ...
    def create_project(self, domain, parent_project):
        ...
        prepared_task = self.create_resource(OpenstackProject, name=name, desc=self.resource.desc, active=False,
                                             attribute={}, parent=None, tags='', is_domain=False, enabled=True,
                                             domain_id=domain, project_id=project_id)
    ...
    def create_security_group(self, parent):
        ...
        prepared_task = self.create_resource(OpenstackSecurityGroup, name=name, desc=self.resource.desc, active=False,
                                             attribute={}, parent=parent_project.oid, tags='')
    ...
    def create_network(self, network_type, vlan, external, private, physical_network=None, public_network=None):
        ...
        if vlan is not None:
            ...
            networks, tot = self.container.get_resources(objdef=OpenstackNetwork.objdef, type=OpenstackNetwork.objdef,
    ...
    def create_subnet(self, cidr, gateway, routes, allocation_pools, enable_dhcp, dns_nameservers, overlap=False):
        ...
        # get network subnets
        net_subnets, tot = self.container.get_resources(objdef=OpenstackSubnet.objdef, type=OpenstackSubnet.objdef,
                                                        parent=network.oid, cidr=cidr)
    ...
    def create_gateway(self, role, uplink_ip_address, transport_ip_address, params):
        ...
        # - get network subnets
        net_subnets, tot = self.container.get_resources(objdef=OpenstackSubnet.objdef, type=OpenstackSubnet.objdef,
                                                        parent=transport_network.oid, cidr=transport_subnet)
        if tot > 0:
            transport_subnet = net_subnets[0].oid
        else:
            self.progress('ERROR - create openstack router: %s' % router_id)
            raise TaskError('no transport network with cidr %s exists in transport network %s' %
        ...
        router = self.get_resource(router.oid)
        prepared_task, code = router.create_port(port_params, sync=True)
    ...
    def create_rule(self, zone, source, destination, service):
        ...
        try:
            ...
            if source['type'] == destination['type'] and source['value'] == destination['value']:
                ...
                policies.append(self.create_openstack_rule(sg_id, 'egress', port_range_min, port_range_max, proto,
                                                           group=sg_id, cidr=None))
                policies.append(self.create_openstack_rule(sg_id, 'ingress', port_range_min, port_range_max, proto,
                                                           group=sg_id, cidr=None))
            ...
            # sgrule1 -> sgrule2
                ...
                policies.append(self.create_openstack_rule(source_sg_id, 'egress', port_range_min, port_range_max,
                                                           proto, group=dest_sg_id, cidr=None))
                policies.append(self.create_openstack_rule(dest_sg_id, 'ingress', port_range_min, port_range_max,
                                                           proto, group=source_sg_id, cidr=None))
            
            # sgrule -> server
            elif source['type'] in ['RuleGroup'] and \
                 destination['type'] in ['Server']:
                # get source security group id
                res = self.get_resource(source['value'])
                source_sg_id = res.get_physical_resource_from_container(self.cid, OpenstackSecurityGroup.objdef).oid
                ...
                policies.append(self.create_openstack_rule(source_sg_id, 'egress', port_range_min, port_range_max,
                                                           proto, group=None, cidr=server_id))
                policies.append(self.create_openstack_rule(dest_sg_id, 'ingress', port_range_min, port_range_max,
                                                           proto, group=source_sg_id, cidr=None))
            ...
            # server -> server
                ...
                policies.append(self.create_openstack_rule(source_sg_id, 'egress', port_range_min, port_range_max,
                                                           proto, group=None, cidr=dest_server_id))
                policies.append(self.create_openstack_rule(dest_sg_id, 'ingress', port_range_min, port_range_max,
                                                           proto, group=None, cidr=source_server_id))
    ...
    def create_openstack_rule(self, sg_id, direction, port_range_min, port_range_max, protocol, group=None, cidr=None):
        ...
        resource_model = self.container.add_resource(objid=objid, name=name, resource_class=CustomResource, ext_id=None,
                                                     active=True, desc=desc, attrib=attribs, parent=None,
                                                     tags=['openstack'])
        ...
        try:
            ...
            if subnet_cidr is not None:
                ...
                if label is not None:
                    ...
                    stype_search_name = 'vlan=%s,protocol=%s,label=%s' % (vlan, proto, label)
    ...
    def create_stack(self, project, params):
        ...
        try:
            ...
            for server in servers:
                ...
                provider_nets = self.task.get_orm_linked_resources(server_net['net_id'], link_type='relation',
                                                                   container_id=None, objdef=None)
                provider_vpc = self.task.get_orm_linked_resources(provider_nets[0].id, link_type='relation.%',
                                                                  container_id=None, objdef=None)
                ...
                for server_sgs in server_sgss:
                    rg = self.task.get_orm_linked_resources(server_sgs['id'], link_type='relation',
                                                            container_id=None, objdef=None)
                    if len(rg) > 0:
                        sg = self.task.get_orm_linked_resources(rg[0].id, link_type='relation.%',
                                                                container_id=None, objdef=None)
    ...
    def create_port(self, network_id, subnet_cidr, fixed_ip, rule_groups):
        ...
        try:
            ...
            # get subnet
            res, total = self.container.get_resources(type=OpenstackSubnet.objdef, parent=remote_net.oid,
                                                      cidr=subnet_cidr)
            if total > 0:
                subnet = res[0]
            else:
    ...
    def create_volume(self, compute_volume, project, params):
        ...
        try:
            ...
            # set volume
                ...
                remote_volumes, tot = volume.get_linked_resources(link_type_filter='relation',
                                                                  run_customize=False, with_permtags=False)
    ...
    def server_action(self, action, params):
        ...
        try:
            ...
            res = self.run_sync_task(prepared_task, msg='Run action %s over server %s' % (action, remote_server.uuid))
            return res
        except Exception as ex:
    ...
    def port_action(self, action, params):
        ...
        try:
            ...
            prepared_task, code = action_func(**params)
            res = self.run_sync_task(prepared_task, msg='Run action %s over port %s' % (action, remote_port.uuid))
            return res
        except Exception as ex:
            self.logger.error(ex, exc_info=False)
            raise TaskError(ex)
    ...
    def volume_action(self, action, params):
        ...
        try:
            ...
            prepared_task, code = action_func(**params)
            res = self.run_sync_task(prepared_task, msg='Run action %s over volume %s' % (action, remote_volume.uuid))
            return res
        except Exception as ex:
            self.logger.error(ex, exc_info=False)
            raise TaskError(ex)
    ...
    def remove_resource(self, childs):
        ...
        try:
            ...
            for child in childs:
                ...
                try:
                    if definition in [OpenstackProject.objdef, OpenstackSecurityGroup.objdef, OpenstackRouter.objdef,
                                      OpenstackServer.objdef, OpenstackHeatStack.objdef, OpenstackPort.objdef,
                                      OpenstackShare.objdef, OpenstackVolume.objdef]:
                        ...
                        for subnet in subnets:
                            prepared_task, code = subnet.expunge(sync=True)
                            self.run_sync_task(prepared_task, msg='delete openstack network subnet %s' % subnet['id'])
                        ...
                        if sub_type == OpenstackSecurityGroupRule.objdef:
                            try:
                                ...
                                self.run_sync_task(prepared_task, msg='remove openstack rule %s' % attribs['id'])
