...
class ComputeLoggingSpaceTask(AbstractProviderResourceTask):
    ...
    def create_zone_logging_space_step(task, step_id, params, availability_zone_id, *args, **kvargs):
        ...
        compute_logging_space.add_link(
            "%s-logging_space-link" % logging_space_id,
            "relation.%s" % site_id,
            logging_space_id,
            attributes={},
        )
        task.progress(
            step_id,
            msg="Link logging_space %s to compute_logging_space %s" % (logging_space_id, oid),
        )
        ...
        task.progress(
            step_id,
            msg="Create logging space %s in availability_zone %s" % (logging_space_id, availability_zone_id),
        )
...
class LoggingSpaceTask(AbstractProviderResourceTask):
    ...
    def create_elk_space_step(task: BaseTask, step_id, params, *args, **kvargs):
        ...
        try:
            ...
            logging_space.add_link(
                "%s-elk_space-link" % space_id,
                "relation",
                space_id,
            ...
            if norescreate is not None and norescreate == True:
                ...
                task.progress(
                    step_id,
                    msg="Link elk_space %s to logging_space %s" % (space_id, oid),
                )
