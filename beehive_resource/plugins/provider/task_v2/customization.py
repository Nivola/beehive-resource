...
from beehive_resource.plugins.provider.entity.customization import ComputeCustomization, Customization
...
class ComputeCustomizationTask(AbstractProviderResourceTask):
    ...
    def create_zone_customization_step(task, step_id, params, availability_zone_id, *args, **kvargs):
        ...
        compute_customization.add_link('%s-customization-link' % customization_id, 'relation.%s' % site_id,
                                       customization_id, attributes={})
        task.progress(step_id, msg='Link customization %s to compute_customization %s' % (customization_id, oid))

        # wait for task to complete
        run_sync_task(prepared_task, task, step_id)
        task.progress(step_id, msg='Create customization %s in availability_zone %s'
