...
class ResourceApiView(SwaggerApiView):
    ...
    def get_resources(self, controller, *args, **kvargs):
        ...
        from beehive_resource.controller import ResourceController
        resourceController: ResourceController = controller
        res, total = resourceController.get_resources(objdef=self.resclass.objdef, type=self.resclass.objdef, *args, **kvargs)
    ...
    def delete_resource(self, controller, oid):
        """
        """
        obj = controller.get_resource(oid, entity_class=self.resclass)
        if obj.get_base_state() not in ['ACTIVE', 'ERROR', 'UNKNOWN']:
            raise ApiManagerError('Resource %s %s is not in a valid state' % (self.resclass.objname, oid), code=400)
        res = obj.delete()
        return res
    ...
    def expunge_resource2(self, controller, oid, **kvargs):
        """
        """
        obj = controller.get_resource(oid, entity_class=self.resclass)
...
class DiscoverResponseSchema(Schema):
    discover_resources = fields.Nested(DiscoverParamsResponseSchema,
