...
class ResourceApiView(ResourceApiViewV1):
    ...
    def __get_resource(self, controller: ResourceController, oid):
        return controller.get_resource(oid, entity_class=self.__get_resource_class(), cache=False)

    def __get_parent_resource(self, controller: ResourceController, oid):
        return controller.get_resource(oid, entity_class=self.__get_parent_resource_class(), cache=False)

    def __exist_resource(self, controller: ResourceController, data, check_name=True):
        ...
        if check_name is True:
            try:
                obj = controller.get_simple_resource(
                    data.get("name"),
                    entity_class=self.__get_resource_class(),
    ...
    def __set_parent(self, controller: ResourceController, data):
        """set parent
    ...
    def clone_resource(self, controller: ResourceController, oid, data, check_name=True):
        """ """
        resclass = self.__get_resource_class_for_create(data)
        res = None, 400
    ...
    def import_resource(self, controller: ResourceController, data, check_name=True):
        """ """
        resclass = self.__get_resource_class_for_create(data)
        if controller.is_class_task_version_v2(resclass):
    ...
    def patch_resource(self, controller: ResourceController, oid, data):
        """ """
    ...
    def delete_resource(self, controller: ResourceController, oid, **kvargs):
        """ """
        obj = self.__get_resource(controller, oid)
        if controller.is_class_task_version_v2(obj.__class__):
    ...
    def expunge_resource(self, controller: ResourceController, oid, **kvargs):
        ...
        if controller.is_class_task_version_v2(obj.__class__):
            ...
            elif str2bool(kvargs.get("force")) is True or obj.get_base_state() in [
                "ACTIVE",
                "ERROR",
                "UNKNOWN",
                "EXPUNGING",
