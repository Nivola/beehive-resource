...
class Container(Base, BaseEntity):
    ...
    tag = relationship(
        "ResourceTag",
        secondary=tags_containers,
        backref=backref("container", lazy="dynamic"),
    )
...
class Resource(Base, BaseEntity):
    ...
    tag = relationship(
        ...
        backref=backref("resource", lazy="dynamic"),
    )
    ...
    def dict_to_model(entity):
        ...
        res = Resource(
            entity.get("objid", None),
            entity.get("name", None),
            res_type,
            None,
            ext_id=entity.get("ext_id", None),
            active=entity.get("active", None),
            desc=entity.get("desc", None),
            attribute=entity.get("attribute", None),
...
class ResourceWithLink(Resource):
    @staticmethod
    def model_to_dict(entity: Resource):
        res = {
            "id": entity.id,
            "uuid": entity.uuid,
            "objid": entity.objid,
    ...
    @staticmethod
    def dict_to_model(entity: dict):
        res_type = ResourceType(entity.get("type_value", None), entity.get("type_objclass", None))
        res_type.id = entity.get("type_id", None)
        res = ResourceWithLink(
            entity.get("objid", None),
            entity.get("name", None),
            res_type,
            None,
...
class ResourceLink(Base, BaseEntity):
    ...
    tag = relationship(
        ...
        backref=backref("resource_link", lazy="dynamic"),
    )

    def __init__(self, objid, name, ltype, start_resource, end_resource, attributes=""):
    ...
    def dict_to_model(entity):
        res = ResourceLink(
            entity.get("objid", None),
            entity.get("name", None),
            entity.get("type", None),
            entity.get("start_resource_id", None),
            entity.get("end_resource_id", None),
...
class ResourceDbManager(AbstractDbManager):
    ...
    def get_tags(self, *args, **kvargs):
        ...
        for tag in tags:
            res.append(
                ResourceTagOccurrences(
                    tag,
                    resources.get(tag.id, 0),
    ...
    def remove_container_type(self, oid=None, value=None):
        ...
        if oid is not None:
            ...
            res = session.query(ContainerType).filter(ContainerType.value.like("%" + value + "%")).first()
        else:
    ...
    def get_containers(self, *args, **kvargs):
        ...
        if kvargs.get("resourcetags", None) is not None:
            custom_select = (
                "(SELECT t1.*, GROUP_CONCAT(t2.name) as tags "
        ...
        res, total = self.get_paginated_entities(
            Container, filters=filters, custom_select=custom_select, *args, **kvargs
        )
        return res, total
    ...
    def get_containers_from_tags(self, *args, **kvargs):
        ...
        :raises QueryError: raise :class:`QueryError`
        """
        tables = [("tags_containers", "t4"), ("resource_tag", "t5")]
    ...
    def add_container(self, objid, name, ctype, connection, desc="", active=True):
        ...
        :raises TransactionError: raise :class:`TransactionError`
        """
        res = self.add_entity(Container, objid, name, ctype, connection, desc=desc, active=active)
        return res
    ...
    def get_container_tags(self, container, *args, **kvargs):
        ...
        """
        tables = [("tags_containers", "t4")]
        filters = ["AND t3.id=t4.tag_id", "AND t4.container_id=:container"]
        res, total = self.get_paginated_entities(
            ResourceTag,
            filters=filters,
            tables=tables,
    ...
    def remove_resource_type(self, oid=None, value=None):
        ...
        if oid is not None:
            ...
            res = session.query(ResourceType).filter(ResourceType.value.like("%" + value + "%"))
        else:
    ...
    def get_resources(self, *args, **kvargs):
        ...
        if kvargs.get("json_attribute_contain", None) is not None:
            ...
            if attribute is not None and isinstance(attribute, dict):
                filters.append(
                    "AND JSON_CONTAINS(`attribute`, '%s', '$.%s')=1" % (attribute.get("value"), attribute.get("field"))
                )
            elif attribute is not None and isinstance(attribute, list):
                for a in attribute:
                    filters.append("AND JSON_CONTAINS(`attribute`, '%s', '$.%s')=1" % (a.get("value"), a.get("field")))
        ...
        res, total = self.get_paginated_entities(
            Resource, filters=filters, custom_select=custom_select, *args, **kvargs
        )
        return res, total
    ...
    def get_resource_links_internal(self, resource):
        ...
        session = self.get_session()
        res = (
            session.query(ResourceLink)
            .filter((ResourceLink.start_resource_id == resource) | (ResourceLink.end_resource_id == resource))
            .all()
        )
    ...
    def get_link_among_resources_internal(self, start, end):
        ...
        res = (
            session.query(ResourceLink)
            .filter((ResourceLink.start_resource_id == start) & (ResourceLink.end_resource_id == end))
            .one()
        )
    ...
    def get_linked_resources_internal(self, resource, link_type=None, container_id=None, objdef=None):
        ...
        session = self.get_session()
        sql = [
            "SELECT t1.id, t1.uuid, t1.objid, t1.name, t1.ext_id,",
        ...
        smtp = text(" ".join(sql))
        fields = self.map_field_to_column(
            [
                "id",
                "objid",
                "name",
    ...
    @query
    def get_directed_linked_resources_internal(
        ...
        session = self.get_session()
        sql = [
            "SELECT t2.start_resource_id as resource," "t1.id, t1.uuid, t1.objid, t1.name, t1.ext_id,",
            "t1.desc, t1.active, t1.parent_id, t1.container_id,",
            "t1.attribute, t1.state, t3.objclass, t3.value as objdef,",
            "t1.creation_date, t1.modification_date, t1.expiry_date,",
    ...
    def get_indirected_linked_resources_internal(
        ...
        session = self.get_session()
        sql = [
            "SELECT t2.end_resource_id as resource," "t1.id, t1.uuid, t1.objid, t1.name, t1.ext_id,",
            "t1.desc, t1.active, t1.parent_id, t1.container_id,",
            "t1.attribute, t1.state, t3.objclass, t3.value as objdef,",
            "t1.creation_date, t1.modification_date, t1.expiry_date,",
            "t2.attributes as link_attr, t2.type as link_type, t2.creation_date as link_creation",
    ...
    def get_resource_tags(self, resource, *args, **kvargs):
        ...
        """
        tables = [("tags_resources", "t4")]
        filters = ["AND t3.id=t4.tag_id", "AND t4.resource_id=:resource"]
        res, total = self.get_paginated_entities(
            ResourceTag,
            filters=filters,
            tables=tables,
    ...
    def is_linked(self, start_resource, end_resource):
        ...
        res = (
            session.query(ResourceLink)
            .filter_by(start_resource_id=start_resource)
    ...
    def get_link_tags(self, link, *args, **kvargs):
        ...
        """
        tables = [("tags_resources_links", "t4")]
        filters = ["AND t3.id=t4.tag_id", "AND t4.link_id=:link"]
        res, total = self.get_paginated_entities(
            ResourceTag, filters=filters, tables=tables, link=link, *args, **kvargs
        )
        return res, total
